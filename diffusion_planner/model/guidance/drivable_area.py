...
def lane_offroad_distance(
    ...
    A = torch.cat([left_pts[:, :-1, :], right_pts[:, :-1, :]], dim=0)  # (2N, K-1, 2)
    B = torch.cat([left_pts[:,  1:, :], right_pts[:,  1:, :]], dim=0)  # (2N, K-1, 2)
    ...
    rel = points.view(M, 1, 1, 1, 2) - quad.view(1, N, K-1, 4, 2)  # (M, N, K-1, 4, 2)
