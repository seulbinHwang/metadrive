...
class BIG:
    ...
    def big_helper_func(self):
        if len(self.blocks
              ) >= self.block_num and self.next_step == NextStep.forward:
            return True
    ...
    def construct(self, block) -> bool:
        success = block.construct_block(self._render_node_path,
                                        self._physics_world)
        lane_num = max([
            len(socket.get_positive_lanes(self._global_network))
