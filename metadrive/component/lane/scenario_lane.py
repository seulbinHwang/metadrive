...
class ScenarioLane(PointLane):
    ...
    def __init__(self, lane_id: int, map_data: dict, need_lane_localization):
        ...
        if ScenarioDescription.POLYGON in map_data[lane_id] and len(
                map_data[lane_id][ScenarioDescription.POLYGON]) > 3:
            polygon = np.asarray(map_data[lane_id][ScenarioDescription.POLYGON])
        else:
            polygon = None
        if "speed_limit_kmh" in map_data[
                lane_id] or "speed_limit_mph" in map_data[lane_id]:
            speed_limit_kmh = map_data[lane_id].get("speed_limit_kmh", None)
            if speed_limit_kmh is None:
                speed_limit_kmh = mph_to_kmh(
                    map_data[lane_id]["speed_limit_mph"])
        else:
        ...
        self.entry_lanes = map_data[lane_id].get(ScenarioDescription.ENTRY,
                                                 None)
        self.exit_lanes = map_data[lane_id].get(ScenarioDescription.EXIT, None)
        self.left_lanes = map_data[lane_id].get(
            ScenarioDescription.LEFT_NEIGHBORS, None)
        self.right_lanes = map_data[lane_id].get(
            ScenarioDescription.RIGHT_NEIGHBORS, None)
    ...
    def try_get_polygon(map_data, lane_id):
        ...
        raise DeprecationWarning(
            "This function can not infer the polygon still")
        ...
        for boundary_info in lane_info[ScenarioDescription.LEFT_BOUNDARIES]:
            boundary = map_data[boundary_info["boundary_feature_id"]][
                ScenarioDescription.POLYLINE]
        ...
        left_boundary_points = np.concatenate(
            left_boundary_points,
            axis=0) if len(left_boundary_points) > 0 else []
        ...
        right_boundary_points = np.concatenate(
            right_boundary_points,
            axis=0) if len(right_boundary_points) > 0 else []
        if len(left_boundary_points) == 0 or len(right_boundary_points) == 0:
            return None
        else:
            polygon = np.concatenate(
                [left_boundary_points, right_boundary_points], axis=0)[..., :2]
    ...
    def get_lane_width(self, lane_id, map_data):
        ...
        if not (ScenarioDescription.RIGHT_NEIGHBORS in map_data[lane_id] and
                ScenarioDescription.LEFT_NEIGHBORS in map_data[lane_id]):
            return self.VIS_LANE_WIDTH
        ...
        if len(right_lanes) + len(left_lanes) == 0:
            return max(sum(map_data[lane_id].get("width", 0)),
                       self.VIS_LANE_WIDTH)
        dist_to_left_lane = 0
        ...
        if len(right_lanes) > 0 and "feature_id" in right_lanes[0]:
            ...
            self_point = map_data[lane_id][
                ScenarioDescription.POLYLINE][self_start]
            dist_to_right_lane = norm(n_point[0] - self_point[0],
                                      n_point[1] - self_point[1])
        if len(left_lanes) > 0 and "feature_id" in left_lanes[0]:
            ...
            self_point = map_data[lane_id][
                ScenarioDescription.POLYLINE][self_start]
            dist_to_left_lane = norm(n_point[0] - self_point[0],
                                     n_point[1] - self_point[1])
