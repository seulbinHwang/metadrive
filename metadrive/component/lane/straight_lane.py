...
class StraightLane(PGLane):
    ...
    def __init__(self,
                 ...
                 line_types: Tuple[PGLineType,
                                   PGLineType] = (PGLineType.BROKEN,
                                                  PGLineType.BROKEN),
                 forbidden: bool = False,
                 speed_limit: float = 1000,
                 priority: int = 0,
                 metadrive_type=MetaDriveType.LANE_SURFACE_STREET) -> None:
        """
        New straight lane.
    ...
    def local_coordinates(self, position: Tuple[float,
        ...
        lateral = delta_x * self.direction_lateral[
            0] + delta_y * self.direction_lateral[1]
        return float(longitudinal), float(lateral)

    def reset_start_end(self, start: Union[np.ndarray, Sequence[float]],
                        end: Union[np.ndarray, Sequence[float]]):
    ...
    def polygon(self):
        if self._polygon is None:
            ...
            for k, lateral in enumerate(
                [+self.width_at(0) / 2, -self.width_at(0) / 2]):
                if k == 1:
                    longs = longs[::-1]
