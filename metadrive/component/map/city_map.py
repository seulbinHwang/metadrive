...
class CityBIG(BIG):
    ...
    def generate(self, generate_method: BigGenerateMethod,
        ...
        if generate_method == BigGenerateMethod.BLOCK_NUM:
            assert isinstance(
                parameter, int
            ), "When generating map by assigning block num, the parameter should be int"
            self.block_num = parameter + 1
        elif generate_method == BigGenerateMethod.BLOCK_SEQUENCE:
            assert isinstance(
                parameter, str
            ), "When generating map from block sequence, the parameter should be a str"
    ...
    def sample_block(self) -> PGBlock:
        ...
        if self._block_sequence is None:
            ...
            block_type = self.np_random.choice(block_types,
        ...
        socket = self.np_random.choice(
            sorted(list(socket_available), key=lambda x: x.index))

        block = block_type(len(self.blocks),
                           socket,
