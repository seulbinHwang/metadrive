...
def parse_map_config(easy_map_config, new_map_config, default_config):
    ...
    if easy_map_config is None or not default_config["map_config"].is_identical(
            new_map_config):
    ...
    if isinstance(easy_map_config, int):
        ...
        raise ValueError(
            "Unkown easy map config: {} and original map config: {}".format(
                easy_map_config, new_map_config))
...
class PGMap(BaseMap):
    ...
    def _big_generate(self, parent_node_path: NodePath,
        ...
        big_map = BIG(
            ...
            block_dist_config=self.engine.global_config.get(
                "block_dist_config", PGBlockDistConfig))
        big_map.generate(self._config[self.GENERATE_TYPE],
    ...
    def _config_generate(self, blocks_config: List, parent_node_path: NodePath,
        ...
        for block_index, b in enumerate(blocks_config[1:], 1):
            block_type = self.engine.global_config[
                "block_dist_config"].get_block(b.pop(self.BLOCK_ID))
        ...
        for _from in map.road_network.graph.keys():
            ...
            for _to in map.road_network.graph[_from].keys():
                for l in map.road_network.graph[_from][_to]:
                    sides = 2 if l is map.road_network.graph[_from][_to][
                        -1] or decoration else 1
