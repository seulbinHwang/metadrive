...
class NodeNetworkNavigation(BaseNavigation):
    ...
    def reset(self, vehicle, dest=None, random_seed=None):
        possible_lanes = ray_localization(vehicle.heading,
                                          vehicle.spawn_place,
                                          self.engine,
        ...
        if len(possible_lanes
              ) == 0 and vehicle.config["spawn_lane_index"] is None:
        ...
        if vehicle.config["spawn_lane_index"] is not None and vehicle.config[
            ...
            idx = possible_lane_indexes.index(
        ...
        destination = self.auto_assign_task(self.map, current_lane.index,
                                            destination, random_seed)
        self.set_route(current_lane.index, destination)

    @staticmethod
    def auto_assign_task(map,
                         current_lane_index,
    ...
    def set_route(self, current_lane_index: str, destination: str):
        ...
        self.checkpoints = self.map.road_network.shortest_path(
            current_lane_index, destination)
        ...
        assert len(
            self.checkpoints) >= 2, "Can not find a route from {} to {}".format(
                current_lane_index[0], destination)
        ...
        if self._dest_node_path is not None:
            ...
            self._dest_node_path.setPos(
                panda_vector(check_point[0], check_point[1], self.MARK_HEIGHT))
        ...
        for ckpt1, ckpt2 in zip(self.checkpoints[:-1], self.checkpoints[1:]):
            self.total_length += self.map.road_network.graph[ckpt1][ckpt2][
                0].length
    ...
    def update_localization(self, ego_vehicle):
        ...
        long_in_ref_lane, _ = self.current_ref_lanes[0].local_coordinates(
            position)
        ...
        if need_update:
            ...
            target_road_1_end = self.checkpoints[
                self._target_checkpoints_index[0] + 1]
            ...
            self._last_long_in_ref_lane = self.current_ref_lanes[
                0].local_coordinates(position)[0]
            ...
            target_road_2_end = self.checkpoints[
        ...
        self._navi_info[:
                        half], lanes_heading1, next_checkpoint = self._get_info_for_checkpoint(
                            lanes_id=0,
                            ref_lane=self.current_ref_lanes[0],
                            ego_vehicle=ego_vehicle)

        # Put the next of the next checkpoint's information into the first half of the navi_info
        self._navi_info[
            half:], lanes_heading2, next_next_checkpoint = self._get_info_for_checkpoint(
                lanes_id=1,
                ref_lane=self.next_ref_lanes[0] if self.next_ref_lanes
                is not None else self.current_ref_lanes[0],
                ego_vehicle=ego_vehicle)
        ...
        if self._show_navi_info:
            ...
            self._draw_line_to_navi(start_position=ego_vehicle.position,
                                    ...
                                    next_checkpoint=(next_navi_pos[0],
                                                     next_navi_pos[1]))

    def _update_target_checkpoints(self, ego_lane_index,
                                   ego_lane_longitude) -> bool:
        """
    ...
    def _get_current_lane(self, ego_vehicle):
        ...
        """
        possible_lanes, on_lane = ray_localization(ego_vehicle.heading,
        ...
        if nx_ckpt == self.checkpoints[-1] or self.next_ref_lanes is None:
            return (*possible_lanes[0][:-1],
                    on_lane) if len(possible_lanes) > 0 else (None, None,
                                                              on_lane)
        ...
        for lane, index, l_1_dist in possible_lanes:
            if lane in next_ref_lanes:
                return lane, index, on_lane
        return (*possible_lanes[0][:-1],
                on_lane) if len(possible_lanes) > 0 else (None, None, on_lane)
    ...
    def _get_info_for_checkpoint(self, lanes_id, ref_lane, ego_vehicle):
        ...
        navi_information.append(
            clip((ckpt_in_heading / self.NAVI_POINT_DIST + 1) / 2, 0.0, 1.0))

        # Dim 2: the relative position of the checkpoint in the target vehicle's right hand side direction.
        navi_information.append(
            clip((ckpt_in_rhs / self.NAVI_POINT_DIST + 1) / 2, 0.0, 1.0))

        if lanes_id == 0:
            lanes_heading = ref_lane.heading_theta_at(
