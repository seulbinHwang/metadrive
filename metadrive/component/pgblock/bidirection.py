...
class Bidirection(PGBlock):
    ...
    def _try_plug_into_previous_block(self) -> bool:
        ...
        fake_positive_lane = ExtendStraightLane(
            basic_lane, length, [PGLineType.BROKEN, PGLineType.SIDE])
        reference_lane = fake_positive_lane
        start_point = reference_lane.position(fake_positive_lane.length,
                                              -reference_lane.width)
        end_point = reference_lane.position(0, -reference_lane.width)
        symmetric_lane = StraightLane(start_point, end_point,
                                      reference_lane.width)
        fake_negative_lane = symmetric_lane

        start_position = basic_lane.position(basic_lane.length,
                                             -basic_lane.width / 2)
        end_position = basic_lane.position(basic_lane.length + length,
                                           -basic_lane.width / 2)
        new_lane = StraightLane(start_position, end_position, basic_lane.width,
        ...
        self.add_sockets(
            BidirectionSocket(fake_positive_lane, fake_negative_lane, socket,
                              _socket))
        return no_cross
