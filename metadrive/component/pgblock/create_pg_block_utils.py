...
def create_extension(start_position: np.ndarray,
                     ...
                     priority: int = 0):
    extension = ExtendingLane(direction, start_position, end_position, width,
                              [PGLineType.NONE, PGLineType.CONTINUOUS],
...
def create_bend_straight(previous_lane: "StraightLane",
    ...
    bend = CircularLane(center, radius, start_phase, angle, clockwise, width,
...
def CreateRoadFrom(
    ...
    if toward_smaller_lane_index:
        ...
        origin_lane.line_types = [
            inner_lane_line_type if len(lanes) > 1 else center_line_type,
            side_lane_line_type
        ]
    else:
    ...
    factor = (PGDrivableAreaProperty.SIDEWALK_WIDTH +
              PGDrivableAreaProperty.SIDEWALK_LINE_DIST +
              lane_width / 2.0) * 2.0 / lane_width
    if not detect_one_side:
        ...
        no_cross = not (
            check_lane_on_road(
                ...
                factor,
                ignore,
                ignore_intersection_checking=ignore_intersection_checking) or
            check_lane_on_road(
                roadnet_to_check_cross,
                lanes[0],
                -0.95,
...
def ExtendStraightLane(lane: "StraightLane",
                       extend_length: float,
                       line_types: (PGLineType, PGLineType),
                       metadrive_lane_type=None) -> "StraightLane":
...
def get_lanes_on_road(road: "Road",
                      roadnet: "NodeRoadNetwork") -> List[AbstractLane]:
...
def CreateAdverseRoad(
        ...
        roadnet_to_check_cross:
    "NodeRoadNetwork",  # mostly, previous global network
        ignore_start: str = None,
        ignore_end: str = None,
    ...
    if isinstance(reference_lane, StraightLane):
        ...
        symmetric_lane = CircularLane(reference_lane.center, radius,
                                      start_phase, angle, new_lane_clockwise,
                                      width, reference_lane.line_types,
                                      reference_lane.forbidden,
                                      reference_lane.speed_limit,
                                      reference_lane.priority)
    else:
    ...
    positive_road.get_lanes(roadnet_to_get_road)[0].line_colors = [
...
def block_socket_merge(socket_1: "BlockSocket", socket_2: "BlockSocket",
...
def create_wave_lanes(pre_lane,
                      lateral_dist: float,
                      wave_length: float,
                      last_straight_length: float,
                      lane_width,
                      toward_left=True):
    """
    ...
    circular_lane_1, pre_lane = create_bend_straight(
        pre_lane, 10, radius, angle, False if toward_left else True, lane_width,
        [PGLineType.NONE, PGLineType.NONE])
    pre_lane.reset_start_end(pre_lane.position(-10, 0),
                             pre_lane.position(pre_lane.length - 10, 0))
    circular_lane_2, straight_lane = create_bend_straight(
        pre_lane, last_straight_length, radius, angle,
