...
class ParkingLot(PGBlock):
    ...
    def _try_plug_into_previous_block(self) -> bool:
        ...
        main_lane = ExtendStraightLane(self.positive_lanes[0],
                                       ...
                                       [PGLineType.BROKEN, PGLineType.NONE])
        ...
        parking_lot_out_lane = ExtendStraightLane(
            main_lane, self.SOCKET_LENGTH, [PGLineType.BROKEN, PGLineType.NONE])
    ...
    def _add_one_parking_space(self, in_socket: PGBlockSocket,
                               out_socket: PGBlockSocket, part_idx: int, radius,
                               dist_to_in, dist_to_out) -> bool:
        no_cross = True
        ...
        bend, straight = create_bend_straight(in_lane,
                                              self.parking_space_length, radius,
                                              self.ANGLE, True,
        ...
        bend_no_cross = CreateRoadFrom(
            ...
            inner_lane_line_type=PGLineType.NONE,
            side_lane_line_type=PGLineType.SIDE
            if dist_to_in < 1e-3 else PGLineType.NONE,
            ignore_intersection_checking=self.ignore_intersection_checking)
        if dist_to_in < 1e-3:
        ...
        bend, straight = create_bend_straight(neg_lane, self.lane_width, radius,
                                              self.ANGLE, False,
                                              self.parking_space_width)
        ...
        CreateTwoWayRoad(
            ...
            inner_lane_line_type=PGLineType.NONE,
            side_lane_line_type=PGLineType.SIDE
            if dist_to_out < 1e-3 else PGLineType.NONE,
            ignore_intersection_checking=self.ignore_intersection_checking)
    ...
    def is_out_direction_parking_space(road: Road):
        ...
        assert (start_node[-2] == "1" and end_node[-2] == "2") or (
            start_node[-2] == "5" and
            end_node[-2] == "6"), "{} to {} is not parking space".format(
                start_node, end_node)
