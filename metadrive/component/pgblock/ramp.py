...
class InRampOnStraight(Ramp):
    ...
    def _get_merge_part(self, att_lane: StraightLane, length: float):
        ...
        merge_part = create_extension(start,
                                      end,
    ...
    def _try_plug_into_previous_block(self) -> bool:
        ...
        extend_road = Road(self.pre_block_socket.positive_road.end_node,
                           self.add_road_node())
        no_cross = CreateRoadFrom(extend_lane,
        ...
        extend_road.get_lanes(self.block_network)[-1].line_types = [
            PGLineType.BROKEN if self.positive_lane_num != 1 else
            PGLineType.CONTINUOUS, PGLineType.CONTINUOUS
        ]
        no_cross = CreateAdverseRoad(extend_road,
                                     self.block_network,
                                     self._global_network,
                                     ignore_intersection_checking=self.
                                     ignore_intersection_checking) and no_cross
        ...
        _extend_road.get_lanes(self.block_network)[-1].line_types = [
            left_lane_line, PGLineType.SIDE
        ]

        # main acc part
        acc_side_lane = ExtendStraightLane(
            extend_lane, length + extension_length,
            [extend_lane.line_types[0], PGLineType.SIDE])
        acc_road = Road(extend_road.end_node, self.add_road_node())
        no_cross = CreateRoadFrom(acc_side_lane,
                                  self.positive_lane_num,
                                  acc_road,
                                  self.block_network,
                                  self._global_network,
                                  ...
                                  ignore_intersection_checking=self.
                                  ignore_intersection_checking) and no_cross
        no_cross = CreateAdverseRoad(acc_road,
                                     self.block_network,
                                     self._global_network,
                                     ignore_intersection_checking=self.
                                     ignore_intersection_checking) and no_cross
        ...
        # socket part
        socket_side_lane = ExtendStraightLane(acc_side_lane, self.SOCKET_LEN,
                                              acc_side_lane.line_types)
        socket_road = Road(acc_road.end_node, self.add_road_node())
        no_cross = CreateRoadFrom(socket_side_lane,
                                  self.positive_lane_num,
                                  socket_road,
                                  self.block_network,
                                  self._global_network,
        ...
        self.set_part_idx(1)
        lateral_dist = (
            1 - cos_angle) * self.RADIUS * 2 + sin_angle * self.CONNECT_PART_LEN
    ...
    def get_intermediate_spawn_lanes(self):
        ...
        """
        spawn_lanes = super(InRampOnStraight,
                            self).get_intermediate_spawn_lanes()
        lane_on_socket = self.get_socket(0).get_positive_lanes(
            self.block_network)[0]
        filtered = []
        ...
        assert sum([
            abs(l.length - InRampOnStraight.RAMP_LEN) <= 0.1
            for ls in spawn_lanes
            for l in ls
        ]) == 1
        return spawn_lanes
...
class OutRampOnStraight(Ramp):
    ...
    def _try_plug_into_previous_block(self) -> bool:
        ...
        dec_road = Road(self.pre_block_socket.positive_road.end_node,
                        self.add_road_node())
        no_cross = CreateRoadFrom(dec_lane,
                                  self.positive_lane_num,
                                  dec_road,
                                  self.block_network,
                                  self._global_network,
                                  ...
                                  ignore_intersection_checking=self.
                                  ignore_intersection_checking) and no_cross
        no_cross = CreateAdverseRoad(dec_road,
                                     self.block_network,
                                     self._global_network,
                                     ignore_intersection_checking=self.
                                     ignore_intersection_checking) and no_cross
        ...
        extend_lane = ExtendStraightLane(
            ...
            [dec_right_lane.line_types[0], PGLineType.CONTINUOUS])
        ...
        _extend_road = -extend_road
        _extend_road.get_lanes(self.block_network)[-1].line_types = [
            PGLineType.NONE if self.positive_lane_num == 1 else
        ...
        next_node = self.add_road_node()
        self.block_network.add_lane(dec_road.start_node, next_node,
                                    merge_part_lane)

        deacc_lane_end = dec_right_lane.position(dec_right_lane.length,
                                                 self.lane_width)
        deacc_lane = StraightLane(merge_part_lane.end, deacc_lane_end,
                                  self.lane_width,
