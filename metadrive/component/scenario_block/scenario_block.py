...
class ScenarioBlock(BaseBlock):
    ...
    def __init__(self, block_index: int, global_network, random_seed, map_index,
                 map_data, need_lane_localization):
        # self.map_data = map_data
        self.need_lane_localization = need_lane_localization
        self.map_index = map_index
        self.map_data = map_data
        super(ScenarioBlock, self).__init__(block_index, global_network,
                                            random_seed)
        ...
        for object_id, data in self.map_data.items():
            if MetaDriveType.is_lane(data.get("type", False)):
                ...
                self.sidewalks[object_id] = {
                    ScenarioDescription.TYPE:
                        MetaDriveType.BOUNDARY_SIDEWALK,
                    ScenarioDescription.POLYGON:
                        np.asarray(data[ScenarioDescription.POLYGON])[..., :2]
                }
    ...
    def create_in_world(self):
        ...
        for lane_id, data in self.map_data.items():
            ...
            if ScenarioDescription.POLYLINE in data and len(
                    data[ScenarioDescription.POLYLINE]) <= 1:
                continue

            if not (MetaDriveType.is_road_line(type) or
                    MetaDriveType.is_road_boundary_line(type)):
                continue

            interval = 2
            line = np.asarray(np.asarray(
                data[ScenarioDescription.POLYLINE]))[..., :2]
            ...
            if MetaDriveType.is_road_line(type):
                if MetaDriveType.is_broken_line(type):
                    self._construct_broken_line(
                        points,
                        PGLineColor.YELLOW if MetaDriveType.is_yellow_line(type)
                        else PGLineColor.GREY)
                else:
    ...
    def _construct_continuous_line(self, points, color):
        for index in range(0, len(points) - 1):
            node_path_list = self._construct_lane_line_segment(
                points[index], points[index + 1], color,
