...
def add_cloud_point_vis(point_x, point_y, height, num_lasers, laser_index,
                        ANGLE_FACTOR, MARK_COLOR0, MARK_COLOR1, MARK_COLOR2):
    f = laser_index / num_lasers if ANGLE_FACTOR else 1
    f *= 0.9
    f += 0.1
    return laser_index, (point_x, point_y,
                         height), (f * MARK_COLOR0, f * MARK_COLOR1,
                                   f * MARK_COLOR2)


def perceive(cloud_points, detector_mask, mask, lidar_range, perceive_distance,
             heading_theta, vehicle_position_x, vehicle_position_y, num_lasers,
             height, physics_world, extra_filter_node, require_colors,
             ANGLE_FACTOR, MARK_COLOR0, MARK_COLOR1, MARK_COLOR2):
    cloud_points.fill(1.0)
    detected_objects = []
    colors = []
    pg_start_position = panda_vector(vehicle_position_x, vehicle_position_y,
                                     height)
    ...
    for laser_index in range(num_lasers):
        if (detector_mask is not None) and (not detector_mask[laser_index]):
            # update vis
            if require_colors:
                point_x, point_y = get_laser_end(lidar_range, perceive_distance,
                                                 laser_index, heading_theta,
                                                 vehicle_position_x,
                                                 vehicle_position_y)
                point_x, point_y, point_z = panda_vector(
                    point_x, point_y, height)
                colors.append(
                    add_cloud_point_vis(point_x, point_y, height, num_lasers,
                                        laser_index, ANGLE_FACTOR, MARK_COLOR0,
        ...
        result = physics_world.rayTestClosest(pg_start_position, laser_end,
                                              mask)
        node = result.getNode()
        if node in extra_filter_node:
            # Fall back to all tests.
            results = physics_world.rayTestAll(pg_start_position, laser_end,
                                               mask)
        ...
        if require_colors:
            colors.append(
                add_cloud_point_vis(laser_end[0], laser_end[1], height,
                                    num_lasers, laser_index, ANGLE_FACTOR,
...
class DistanceDetector(BaseSensor):
    ...
    def __init__(self, engine):
        ...
        self.cloud_points_vis = ColorLineNodePath(
            self.origin,
            thickness=3.0) if AssetLoader.loader is not None else None
        self.logger.debug("Load Vehicle Module: {}".format(
    ...
    def perceive(self,
        ...
        assert not isinstance(
            detector_mask, str
        ), "Please specify detector_mask either with None or a numpy array."
        cloud_points, detected_objects, colors = perceive(
            cloud_points=np.ones((num_lasers,), dtype=float),
            detector_mask=detector_mask.astype(
                dtype=np.uint8) if detector_mask is not None else None,
            mask=self.mask,
            lidar_range=self._get_lidar_range(num_lasers,
        ...
        if show and self.cloud_points_vis is not None:
            ...
            self.cloud_points_vis.draw_lines(
                [[p[1] for p in colors]],
                [[LVecBase4(*p[-1], 1) for p in colors[1:]]])

        return detect_result(cloud_points=cloud_points.tolist(),
