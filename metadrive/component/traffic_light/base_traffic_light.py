...
class BaseTrafficLight(BaseObject):
    ...
    def __init__(
        ...
        super(BaseTrafficLight, self).__init__(name, random_seed, config,
        ...
        self.add_body(air_wall, add_to_static_world=False
                     )  # add to dynamic world so the lidar can detect it
        ...
        self.set_heading_theta(
            lane.heading_theta_at(self.PLACE_LONGITUDE) if lane else 0)
        self.current_light = None

        if self.render:
            if len(BaseTrafficLight.TRAFFIC_LIGHT_MODEL
                  ) == 0 and self._show_model:
                for color in ["green", "red", "yellow", "unknown"]:
                    model = self.loader.loadModel(
                        AssetLoader.file_path("models", "traffic_light",
                                              "{}.gltf".format(color)))
            ...
            if self._draw_line:
                ...
                self._lane_center_line = np.array(
                    [[p[0], p[1], 0.4] for p in self.lane.get_polyline()])
    ...
    def _try_draw_line(self, color):
        if self._draw_line:
            ...
            self._line_drawer.draw_lines(
                [self._lane_center_line],
                [[color for _ in self._lane_center_line]])
    ...
    def set_green(self):
        if self.render:
            ...
            if self._show_model:
                self.current_light = BaseTrafficLight.TRAFFIC_LIGHT_MODEL[
                    "green"].instanceTo(self.origin)
            self._try_draw_line([3 / 255, 255 / 255, 3 / 255])
        self.status = MetaDriveType.LIGHT_GREEN
        self._body.setIntoCollideMask(
            CollisionGroup.AllOff)  # can not be detected by anything
    ...
    def set_red(self):
        if self.render:
            ...
            if self._show_model:
                self.current_light = BaseTrafficLight.TRAFFIC_LIGHT_MODEL[
                    "red"].instanceTo(self.origin)
            self._try_draw_line([252 / 255, 0 / 255, 0 / 255])
        self.status = MetaDriveType.LIGHT_RED
        self._body.setIntoCollideMask(
            CollisionGroup.InvisibleWall
        )  # will be detected by lidar and object detector
    ...
    def set_unknown(self):
        if self.render:
            ...
            if self._show_model:
                self.current_light = BaseTrafficLight.TRAFFIC_LIGHT_MODEL[
                    "unknown"].instanceTo(self.origin)
        self.status = MetaDriveType.LIGHT_UNKNOWN
        self._body.setIntoCollideMask(
            CollisionGroup.AllOff)  # can not be detected by anything
