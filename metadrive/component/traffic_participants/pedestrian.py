...
class Pedestrian(BaseTrafficParticipant):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(Pedestrian, self).__init__(position,
                                         heading_theta,
                                         random_seed,
                                         name=name)
        ...
        if self.render:
            ...
            self._instance = Pedestrian._MODEL[
                self.current_speed_model].instanceTo(self.origin)
    ...
    def reset(self,
              ...
              *args,
              **kwargs):
        super(Pedestrian, self).reset(position, heading_theta, random_seed,
                                      name, *args, **kwargs)
        self.current_speed_model = self.SPEED_LIST[0]
    ...
    def init_pedestrian_model(cls):
        for idx, speed in enumerate(cls.SPEED_LIST):
            model = Actor(
                AssetLoader.file_path("models", "pedestrian", "scene.gltf"))
    ...
    def set_velocity(self, direction: list, value=None, in_local_frame=False):
        ...
        if self.render:
            speed_model_index = self.get_speed_model(
                target_speed=speed if value is None else value)
            if speed_model_index != self.current_speed_model:
                self._instance.detachNode()
                self._instance = Pedestrian._MODEL[
                    speed_model_index].instanceTo(self.origin)
...
class PedestrianBoundingBox(BaseTrafficParticipant):
    ...
    def __init__(self,
        ...
        super(PedestrianBoundingBox, self).__init__(position,
                                                    heading_theta,
                                                    random_seed,
                                                    name=name,
                                                    config=config)
        ...
        # self.body.addShape(BulletCylinderShape(self.RADIUS, self.HEIGHT))
        self.body.addShape(
            BulletBoxShape(LVector3(width / 2, length / 2, height / 2)))
