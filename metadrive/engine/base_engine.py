...
class BaseEngine(EngineCore, Randomizable):
    ...
    def __init__(self, global_config):
        ...
        self._num_scenarios_per_level = int(
            self.global_config.get("num_scenarios", 1) / self._max_level)
    ...
    def add_policy(self, object_id, policy_class, *args, **kwargs):
        ...
        if self.record_episode:
            ...
            self.record_manager.add_policy_info(object_id, policy_class, *args,
                                                **kwargs)
        return policy
    ...
    def has_policy(self, object_id, policy_cls=None):
        if policy_cls is None:
            ...
            return True if object_id in self._object_policies and isinstance(
                self._object_policies[object_id], policy_cls) else False
    ...
    def spawn_object(self,
        ...
        if force_spawn or object_class.__name__ not in self._dying_objects or len(
            ...
            obj.reset(**kwargs)
            if not is_map_related_class(object_class) and (
        ...
        if self.global_config[
                "record_episode"] and not self.replay_episode and record:
            self.record_manager.add_spawn_info(obj, object_class, kwargs)
        self._spawned_objects[obj.id] = obj
        color = self._pick_color(obj.id)
        if color == (-1, -1, -1):
            raise ValueError(
                "No color available for object: {} instance segment mask. We already used all {} colors..."
                .format(obj.id, BaseEngine.MAX_COLOR))
    ...
    def clear_objects(self,
                      filter: Optional[Union[Callable, List]],
                      force_destroy=False,
                      record=True):
        """
        ...
        """
        force_destroy_this_obj = True if force_destroy or self.global_config[
            "force_destroy"] else False

        if isinstance(filter, (list, tuple)):
            exclude_objects = {
                obj_id: self._spawned_objects[obj_id] for obj_id in filter
            }
        elif callable(filter):
        ...
        for id, obj in exclude_objects.items():
            ...
            if force_destroy_this_obj:
                ...
                if len(self._dying_objects[obj.class_name]
                      ) < self.global_config["num_buffering_objects"]:
    ...
    def reset(self):
        ...
        self.replay_episode = True if self.global_config[
            "replay_episode"] is not None else False
        self.record_episode = self.global_config["record_episode"]
        self.only_reset_when_replay = self.global_config[
            "only_reset_when_replay"]
    ...
    def register_manager(self, manager_name: str, manager):
        ...
        assert not hasattr(
            self, manager_name
        ), "Manager name can not be same as the attribute in BaseEngine"
        self._managers[manager_name] = manager
        setattr(self, manager_name, manager)
        self._managers = OrderedDict(
            sorted(self._managers.items(), key=lambda k_v: k_v[-1].PRIORITY))

    def seed(self, random_seed):
        start_seed = self.gets_start_index(self.global_config)
        random_seed = ((random_seed - start_seed) %
                       self._num_scenarios_per_level) + start_seed
    ...
    def gets_start_index(config):
        ...
        if start_seed is not None:
            ...
            logger.warning(
                "Can not find `start_seed` or `start_scenario_index`. Use 0 as `start_seed`"
            )
            return 0
    ...
    def setup_main_camera(self):
        ...
        if self.mode != RENDER_MODE_NONE:
            return MainCamera(self, self.global_config["camera_height"],
                              self.global_config["camera_dist"])
        else:
            return None
    ...
    def _object_clean_check(self):
        ...
        objs_need_to_release = self.get_objects(filter=lambda obj: isinstance(
            obj, BaseVehicle) or isinstance(obj, TrafficObject))
        assert len(
        ...
        bodies = []
        for world in [
                self.physics_world.dynamic_world,
                self.physics_world.static_world
        ]:
        ...
        assert len(
            children
        ) == 0, "NodePath are not cleaned thoroughly. Remaining NodePath: {}".format(
            children)

    def update_manager(self,
                       manager_name: str,
                       manager,
                       destroy_previous_manager=True):
        """
    ...
    def _get_window_image(self, return_bytes=False):
        ...
        texture = self.graphicsEngine.getWindow(window_count).getDisplayRegion(
            0).getScreenshot()

        assert texture.getXSize() == self.global_config["window_size"][0], (
            texture.getXSize(), texture.getYSize(),
            self.global_config["window_size"])
    ...
    def warmup(self):
        ...
        """
        if self.global_config[
                "preload_models"] and self.mode != RENDER_MODE_NONE:
            ...
            warm_up_pedestrian = self.spawn_object(Pedestrian,
                                                   position=[0, 0],
                                                   heading_theta=0,
                                                   record=False)
            warm_up_light = self.spawn_object(BaseTrafficLight,
                                              lane=None,
                                              position=[0, 0],
                                              record=False)
            barrier = self.spawn_object(TrafficBarrier,
                                        position=[0, 0],
                                        heading_theta=0,
                                        record=False)
            cone = self.spawn_object(TrafficCone,
                                     position=[0, 0],
                                     heading_theta=0,
                                     record=False)
            for vel in Pedestrian.SPEED_LIST:
                ...
                self.taskMgr.step()
            self.clear_objects(
                [warm_up_pedestrian.id, warm_up_light.id, barrier.id, cone.id],
                record=False)
    ...
    def get_task(self, object_id):
        ...
        """
        raise DeprecationWarning
        assert object_id in self._object_tasks, "Can not find the task for object(id: {})".format(
            object_id)
...
if __name__ == "__main__":
    ...
    car_model = world.loader.loadModel(
        AssetLoader.file_path("models", "vehicle", "lada", "vehicle.gltf"))
