...
class Terrain(BaseObject, ABC):
    ...
    PATH = pathlib.PurePosixPath(__file__).parent if not is_win(
    ...
    def __init__(self, show_terrain, engine):
        ...
        self._height_scale = engine.global_config[
            "height_scale"] * self._terrain_size / 2048  # [m]
        self._drivable_area_extension = engine.global_config[
            "drivable_area_extension"]  # [m] road marin
        ...
        self._heightfield_start = int(
            (self._terrain_size - self._heightmap_size) / 2)
        self._semantic_map_pixel_per_meter = TerrainProperty.get_semantic_map_pixel_per_meter(
        )  # [m]  pixels per meter
        ...
        if self.render:
            ...
            self.semantic_tex.setup2dTexture(semantic_size, semantic_size,
                                             Texture.TUnsignedByte,
                                             Texture.F_red)

            # prepare height field texture
            self.heightfield_tex = Texture()
            self.heightfield_tex.setup2dTexture(*self.heightfield_img.shape[:2],
                                                Texture.TShort,
                                                Texture.FLuminance)
    ...
    def reset(self, center_point):
        ...
        if self.render or self.use_mesh_terrain:
            ...
            assert (drivable_region
                    is None) or (isinstance(drivable_region, np.ndarray) and
                                 drivable_region.dtype == bool)
            ...
            if drivable_region is None:
                drivable_area_height_mean = self.heightfield_img[start:end,
                                                                 start:end,
                                                                 ...].mean()
                heightfield_base = np.where(
                    heightfield_base
                    > (drivable_area_height_mean - self._terrain_offset),
                    heightfield_base -
                    (drivable_area_height_mean - self._terrain_offset),
                    0).astype(np.uint16)
                ...
                heightfield_base[start:end, start:end,
                                 ...] = self._terrain_offset
                logger.warning("No map is found in map region, "
                               "size: [{}, {}], "
                               "center: {}".format(self._semantic_map_size,
                                                   self._semantic_map_size,
                                                   center_point))
            else:
                drivable_area_height_mean = np.mean(
                    self.heightfield_img[start:end, start:end,
                                         ...][np.where(drivable_region)])
                heightfield_base = np.where(
                    heightfield_base
                    > (drivable_area_height_mean - self._terrain_offset),
                    heightfield_base -
            ...
            if self.use_mesh_terrain:
                self._generate_collision_mesh(
                    heightfield_base if self.full_size_mesh else
                    heightfield_to_modify, self._height_scale)
    ...
    def _generate_mesh_vis_terrain(
        ...
        assert isinstance(heightfield,
                          Texture), "heightfield file must be Panda3D Texture"
        ...
        self._mesh_terrain = self.origin.attach_new_node(
            self._mesh_terrain_node)
        # shader is determined by tag state, enabling multi-pass rendering
        self._mesh_terrain.setTag(CameraTagStateKey.Semantic,
                                  self.SEMANTIC_LABEL)
    ...
    def _set_terrain_shader(self, engine, attribute_tex):
        ...
        if not self._terrain_shader_set:
            if engine.use_render_pipeline:
                ...
                terrain_effect = AssetLoader.file_path("../shaders",
                                                       "terrain_effect.yaml")
                engine.render_pipeline.set_effect(self._mesh_terrain,
                                                  terrain_effect, {}, 100)

            # # height
            self._mesh_terrain.set_shader_input("camera", self.engine.camera)
            self._mesh_terrain.set_shader_input("height_scale",
                                                self._height_scale)

            # grass
            self._mesh_terrain.set_shader_input("grass_tex", self.grass_tex)
            self._mesh_terrain.set_shader_input("grass_normal",
                                                self.grass_normal)
            self._mesh_terrain.set_shader_input("grass_rough", self.grass_rough)
            self._mesh_terrain.set_shader_input("grass_tex_ratio",
            ...
            self._mesh_terrain.set_shader_input("rock_tex_ratio",
                                                self.rock_tex_ratio)

            # rock 2
            self._mesh_terrain.set_shader_input("rock_tex_2", self.rock_tex_2)
            self._mesh_terrain.set_shader_input("rock_normal_2",
                                                self.rock_normal_2)
            self._mesh_terrain.set_shader_input("rock_rough_2",
                                                self.rock_rough_2)
            self._mesh_terrain.set_shader_input("rock_tex_ratio_2",
                                                self.rock_tex_ratio_2)

            # road
            self._mesh_terrain.set_shader_input("road_tex", self.road_texture)
            self._mesh_terrain.set_shader_input("road_tex_ratio",
                                                self.road_tex_ratio)
            self._mesh_terrain.set_shader_input("road_normal",
                                                self.road_texture_normal)
            self._mesh_terrain.set_shader_input("road_rough",
                                                self.road_texture_rough)

            # crosswalk
            self._mesh_terrain.set_shader_input("crosswalk_tex",
                                                self.crosswalk_tex)
    ...
    def _generate_collision_mesh(self, heightfield_img, height_scale):
        ...
        path_to_store = self.PATH.joinpath("run_time_map_mesh_{}.png".format(
            self.engine.pid))
        ...
        p = PNMImage(
            Filename(
                str(
                    AssetLoader.windows_style2unix_style(path_to_store)
                    if is_win() else path_to_store)))
    ...
    def set_position(self, position, height=None):
        ...
        """
        if self.render:
            pos = (self._mesh_terrain.get_pos()[0] + position[0],
                   self._mesh_terrain.get_pos()[1] + position[1])
            self._mesh_terrain.set_pos(*pos, 0)
            if self.probe is not None:
                self.probe.set_pos(*pos, self.PROBE_HEIGHT)
        if self.use_mesh_terrain:
            self.mesh_collision_terrain.set_pos(*position[:2],
                                                self._height_scale)
    ...
    def _generate_card_terrain(self):
        ...
        self.origin.hide(CamMask.MiniMap | CamMask.Shadow | CamMask.DepthCam |
                         CamMask.ScreenshotCam | CamMask.SemanticCam)
        ...
        card.setQuat(
            LQuaternionf(math.cos(-math.pi / 4), math.sin(-math.pi / 4), 0, 0))

    def _load_height_field_image(self, engine):
        # basic height_field
        heightfield_tex = engine.loader.loadTexture(
            AssetLoader.file_path("textures", "terrain", "heightfield.png"))
        heightfield_img = np.frombuffer(heightfield_tex.getRamImage().getData(),
                                        dtype=np.uint16)
        heightfield_img = heightfield_img.reshape(
            (heightfield_tex.getYSize(), heightfield_tex.getXSize(), 1))
        # heightfield_img = numpy.rot90(heightfield_img)
        down_sample_rate = int(heightfield_tex.getYSize() /
                               self._terrain_size)  # downsample to 2048 m
        self.heightfield_img = np.array(
            heightfield_img[::down_sample_rate, ::down_sample_rate])

    def _load_mesh_terrain_textures(self,
                                    engine,
        ...
        self.rock_tex = self.loader.loadTexture(
            AssetLoader.file_path("textures", "rock",
                                  "brown_mud_leaves_01_diff_1k.png"))
        self.rock_normal = self.loader.loadTexture(
            AssetLoader.file_path("textures", "rock",
                                  "brown_mud_leaves_01_nor_gl_1k.png"))
        self.rock_rough = self.loader.loadTexture(
            AssetLoader.file_path("textures", "rock",
                                  "brown_mud_leaves_01_rough_1k.png"))
        ...
        self.rock_tex_2 = self.loader.loadTexture(
            AssetLoader.file_path("textures", "grass2",
                                  "grass_path_2_diff_1k.png"))
        self.rock_normal_2 = self.loader.loadTexture(
            AssetLoader.file_path("textures", "grass2",
                                  "grass_path_2_nor_gl_1k.png"))
        self.rock_rough_2 = self.loader.loadTexture(
            AssetLoader.file_path("textures", "grass2",
                                  "grass_path_2_rough_1k.png"))
        ...
        self.road_texture_rough = self.loader.loadTexture(
            AssetLoader.file_path("textures", "asphalt", "rough_2k.png"))
        self.road_tex_ratio = 128 * self._terrain_size / 2048
        v_wrap = Texture.WMRepeat
        u_warp = Texture.WMMirror
        filter_type = Texture.FTLinearMipmapLinear
        for tex in [
                self.road_texture_rough, self.road_texture,
                self.road_texture_normal
        ]:
            tex.set_wrap_u(u_warp)
            tex.set_wrap_v(v_wrap)
        ...
        # crosswalk
        tex = np.frombuffer(self.road_texture.getRamImage().getData(),
                            dtype=np.uint8)
        tex = tex.copy()
        tex = tex.reshape(
            (self.road_texture.getYSize(), self.road_texture.getXSize(), 3))
        step_size = 64
        for x in range(0, 2048, step_size * 2):
            tex[x:x + step_size, ...] = 220
        self.crosswalk_tex = Texture()
        self.crosswalk_tex.setup2dTexture(*tex.shape[:2], Texture.TUnsignedByte,
                                          Texture.F_rgb)
    ...
    def _make_random_terrain(self, texture_size, terrain_size, heightfield):
        ...
        """
        raise DeprecationWarning
        height_1 = width_2 = height_3 = width_3 = length = int(
            (terrain_size - texture_size) / 2)
        width_1 = height_2 = width = texture_size
        max_height = 8192
        min_height = 0
        roughness = 0.14

        array_1 = diamond_square([height_1, width_1],
                                 min_height,
                                 max_height,
                                 roughness,
                                 random_seed=self.generate_seed())
        array_2 = diamond_square([height_2, width_2],
                                 min_height,
                                 max_height,
                                 roughness,
                                 random_seed=self.generate_seed())

        array_3 = diamond_square([height_3, width_3],
                                 min_height,
                                 max_height,
                                 roughness,
                                 random_seed=self.generate_seed())
    ...
    def get_drivable_region(self, center_point):
        ...
        if self.engine.current_map:
            drivable_region = self.engine.current_map.get_height_map(
                center_point, self._heightmap_size, 1,
    ...
    def get_terrain_semantics(self, center_point):
        ...
        if self.engine.current_map:
            ...
            lane_color = MapTerrainSemanticColor.get_color(
                MetaDriveType.LANE_SURFACE_STREET)
            semantics = np.ones(
                (size, size, 1), dtype=np.uint8) * lane_color  # use lane color
        return semantics
