...
class MABottleneckPGMapManager(PGMapManager):
    ...
    def reset(self):
        ...
        if len(self.spawned_objects) == 0:
            _map = self.spawn_object(MABottleneckMap,
...
class MultiAgentBottleneckEnv(MultiAgentMetaDrive):
    ...
    def default_config() -> Config:
        assert MABottleneckConfig["vehicle_config"]["side_detector"][
            "num_lasers"] > 2
        assert MABottleneckConfig["vehicle_config"]["lane_line_detector"][
            "num_lasers"] > 2
        MABottleneckConfig["map_config"]["lane_num"] = MABottleneckConfig[
            "map_config"]["bottle_lane_num"]
        return MultiAgentMetaDrive.default_config().update(
            MABottleneckConfig, allow_add_new_key=True)
