...
class MAIntersectionSpawnManager(SpawnManager):
    ...
    def update_destination_for(self, agent_id, vehicle_config):
        ...
        if self.disable_u_turn:  # Remove the spawn road from end roads
            end_roads = [
                r for r in end_roads
                if Road(*vehicle_config["spawn_lane_index"][:2]) != r
            ]
...
class MAIntersectionPGMapManager(PGMapManager):
    ...
    def reset(self):
        ...
        if len(self.spawned_objects) == 0:
            _map = self.spawn_object(MAIntersectionMap,
...
class MultiAgentIntersectionEnv(MultiAgentMetaDrive):
    ...
    def default_config() -> Config:
        return MultiAgentMetaDrive.default_config().update(
            MAIntersectionConfig, allow_add_new_key=True)
    ...
    def setup_engine(self):
        ...
        self.engine.update_manager(
            "spawn_manager",
            MAIntersectionSpawnManager(disable_u_turn=disable_u_turn))
...
def show_map_and_traj():
    ...
    with open("metasvodist_inter_best.json", "r") as f:
        traj = json.load(f)
    m = draw_top_down_map_native(env.current_map,
                                 semantic_map=False,
                                 return_surface=True)
