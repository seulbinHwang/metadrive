...
class CommunicationObservation(LidarStateObservation):
    ...
    def observation_space(self):
        ...
        if self.config["vehicle_config"]["lidar"]["num_lasers"] > 0 \
            ...
            lidar_dim += self.env.num_agents * (
                5 + (4 if self.config["lidar"]["add_others_navi"] else 0))
            shape[0] += lidar_dim
    ...
    def __init__(self, vehicle_config, env):
        ...
        super(CommunicationObservation,
    ...
    def refresh_agent_name_index_mapping(self):
        ...
        name_vehicle_mapping = {
            k: name_vehicle_mapping[k]
            for k in sorted(name_vehicle_mapping.keys())
        }
        ...
        new_agent_names = new_agents.difference(
            self.agent_name_index_mapping.keys())
        for k in new_agent_names:
            ...
            index = (index + 1) / (
                num_agents)  # scale to [1/num_agents, 1]. 0 left to no vehicle.
            self.agent_name_index_mapping[k] = index

        # assert len(new_agents.symmetric_difference(self.agent_name_index_mapping.keys())) == 0
        assert all(
            k in self.agent_name_index_mapping for k in name_vehicle_mapping)
        assert all(k in self.agent_name_slot_mapping.keys()
                   for k in name_vehicle_mapping)
        self.agent_name_index_mapping = {
    ...
    def get_global_info(self, ego_vehicle):
        ...
        for agent_name, vehicle in name_vehicle_mapping.items():
            ...
            res[slot_index *
                res_size] = self.agent_name_index_mapping[agent_name]

            # assert isinstance(vehicle, IDMVehicle or Base), "Now MetaDrive Doesn't support other vehicle type"

            relative_position = ego_vehicle.convert_to_local_coordinates(
                vehicle.position, ego_position)
            relative_position = self._process_norm(relative_position,
                                                   perceive_distance)
            ...
            res[slot_index * res_size + 1] = clip(
                (relative_position[0] / perceive_distance + 1) / 2, 0.0, 1.0)
            res[slot_index * res_size + 2] = clip(
                (relative_position[1] / perceive_distance + 1) / 2, 0.0, 1.0)

            relative_velocity = ego_vehicle.convert_to_local_coordinates(
                vehicle.velocity, ego_vehicle.velocity)
            relative_velocity = self._process_norm(relative_velocity,
                                                   speed_scale)
            res[slot_index * res_size + 3] = clip(
                (relative_velocity[0] / speed_scale + 1) / 2, 0.0, 1.0)
            res[slot_index * res_size + 4] = clip(
                (relative_velocity[1] / speed_scale + 1) / 2, 0.0, 1.0)

            if add_others_navi:
                ckpt1, ckpt2 = vehicle.navigation.get_checkpoints()

                relative_ckpt1 = ego_vehicle.lidar._project_to_vehicle_system(
                    ckpt1, ego_vehicle, perceive_distance)
                relative_ckpt1 = self._process_norm(relative_ckpt1,
                                                    perceive_distance)
                res[slot_index * res_size + 5] = clip(
                    (relative_ckpt1[0] / perceive_distance + 1) / 2, 0.0, 1.0)
                res[slot_index * res_size + 6] = clip(
                    (relative_ckpt1[1] / perceive_distance + 1) / 2, 0.0, 1.0)

                relative_ckpt2 = ego_vehicle.lidar._project_to_vehicle_system(
                    ckpt2, ego_vehicle)
                relative_ckpt2 = self._process_norm(relative_ckpt2,
                                                    perceive_distance,
                                                    perceive_distance)
                res[slot_index * res_size + 7] = clip(
                    (relative_ckpt2[0] / perceive_distance + 1) / 2, 0.0, 1.0)
                res[slot_index * res_size + 8] = clip(
                    (relative_ckpt2[1] / perceive_distance + 1) / 2, 0.0, 1.0)
...
class TinyInterRuleBasedPolicy(IDMPolicy):
    ...
    def __init__(self, control_object, random_seed, target_speed=10):
        super(TinyInterRuleBasedPolicy,
...
class MixedIDMAgentManager(VehicleAgentManager):
    ...
    def __init__(self,
                 init_observations,
                 num_RL_agents,
                 ignore_delay_done=None,
                 target_speed=10):
        super(MixedIDMAgentManager,
              self).__init__(init_observations=init_observations)
    ...
    def filter_RL_agents(self, source_dict, original_done_dict=None):
        ...
        new_ret = {
            ...
            if k in self.all_previous_RL_agents
        }
    ...
    def get_observation_spaces(self):
        ret = self.filter_RL_agents(
            super(MixedIDMAgentManager, self).get_observation_spaces())
        if len(ret) == 0:
            k, v = list(
                super(MixedIDMAgentManager,
                      self).get_observation_spaces().items())[0]
            return {k: v}
        else:
            return ret
    ...
    def get_action_spaces(self):
        ret = self.filter_RL_agents(
            super(MixedIDMAgentManager, self).get_action_spaces())
        if len(ret) == 0:
            k, v = list(
                super(MixedIDMAgentManager,
                      self).get_action_spaces().items())[0]
            return {k: v}
        else:
            return ret
...
class MultiAgentTinyInter(MultiAgentIntersectionEnv):
    ...
    def default_config() -> Config:
        ...
        return MultiAgentIntersectionEnv.default_config().update(
            tiny_config, allow_add_new_key=True)
    ...
    def step(self, actions):
        ...
        d = self.agent_manager.filter_RL_agents(
            tm, original_done_dict=original_done_dict)
        if "__all__" in d:
            d.pop("__all__")
        # assert len(d) == self.agent_manager.num_RL_agents, d
        d["__all__"] = all(d.values())
        return (
            self.agent_manager.filter_RL_agents(
                o, original_done_dict=original_done_dict),
            self.agent_manager.filter_RL_agents(
                r, original_done_dict=original_done_dict),
            d,
            self.agent_manager.filter_RL_agents(
                tc, original_done_dict=original_done_dict),
            self.agent_manager.filter_RL_agents(
                i, original_done_dict=original_done_dict),
        )
    ...
    def _preprocess_actions(self, actions):
        ...
        actions = {
            v_id: actions[v_id]
            for v_id in self.agents.keys()
            if v_id in self.agent_manager.RL_agents
        }
        return actions
...
if __name__ == '__main__':
    ...
    for i in range(1, 100000):
        o, r, tm, tc, info = env.step(
            {k: [0.0, 0.0] for k in env.action_space.sample().keys()})
        ...
        if tm["__all__"]:
            ...
            print(
                ...
                .format(
                    ...
                    ep_success_reward_sum /
                    ep_success if ep_success > 0 else -1,
                    (ep_reward_sum - ep_success_reward_sum) /
                    (ep_done - ep_success) if
