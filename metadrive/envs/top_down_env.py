...
class TopDownSingleFrameMetaDriveEnv(MetaDriveEnv):
    ...
    def get_single_observation(self, _=None):
        return TopDownObservation(self.config["vehicle_config"],
...
class TopDownMetaDrive(TopDownSingleFrameMetaDriveEnv):
    ...
    def get_single_observation(self, _=None):
        return TopDownMultiChannel(self.config["vehicle_config"],
                                   onscreen=self.config["use_render"],
                                   clip_rgb=self.config["norm_pixel"],
                                   frame_stack=self.config["frame_stack"],
                                   post_stack=self.config["post_stack"],
                                   frame_skip=self.config["frame_skip"],
                                   resolution=(self.config["resolution_size"],
                                               self.config["resolution_size"]),
                                   max_distance=self.config["distance"])
...
if __name__ == '__main__':
    ...
    env = TopDownMetaDriveEnvV2(
        dict(num_scenarios=1, start_seed=5000, distance=30))
    ...
    env.reset()
    names = [
        "road_network", "navigation", "past_pos", "traffic t", "traffic t-1",
        "traffic t-2", "traffic t-3", "traffic t-4"
    ]
    for _ in range(60):
