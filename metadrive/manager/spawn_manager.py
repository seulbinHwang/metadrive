...
class SpawnManager(BaseManager):
    ...
    def __init__(self):
        ...
        self.exit_length = (
            self.engine.global_config["map_config"]["exit_length"] -
            FirstPGBlock.ENTRANCE_LENGTH)
        assert self.exit_length >= self.RESPAWN_REGION_LONGITUDE, (
            "The exist length {} should greater than minimal longitude interval {}."
            .format(self.exit_length, self.RESPAWN_REGION_LONGITUDE))
        ...
        agent_configs, safe_spawn_places = self._auto_fill_spawn_roads_randomly(
            spawn_roads)
    ...
    def reset(self):
        ...
        if num_agents == -1:  # Infinite number of agents
            ...
            target_agents = self.np_random.choice(
                [i for i in range(len(self.available_agent_configs))],
                num_agents,
                replace=False)
        ...
        if len(target_agents) > 1:
            ...
            ret["agent0"] = self._randomize_position_in_slot(
                self.available_agent_configs[0]["config"])
        ...
        for agent_id, config in ret.items():
            ...
            if not config.get("destination",
                              False) or config["destination"] is None:
                config = self.update_destination_for(agent_id, config)
            agent_configs[agent_id] = config

        self.engine.global_config["agent_configs"] = copy.deepcopy(
            agent_configs)
    ...
    def max_capacity(spawn_roads, exit_length, lane_num):
        ...
        num_slots = int(
            floor(exit_length / SpawnManager.RESPAWN_REGION_LONGITUDE))
        assert num_slots > 0, "The exist length {} should greater than minimal longitude interval {}.".format(
            exit_length, SpawnManager.RESPAWN_REGION_LONGITUDE +
            FirstPGBlock.ENTRANCE_LENGTH)
    ...
    def _auto_fill_spawn_roads_randomly(self, spawn_roads):
        ...
        num_slots = int(
            floor(self.exit_length / SpawnManager.RESPAWN_REGION_LONGITUDE))
        ...
        if self.num_agents is not None:
            ...
            assert self.num_agents <= self.max_capacity(
                spawn_roads, self.exit_length + FirstPGBlock.ENTRANCE_LENGTH,
                self.lane_num
            ), ("Too many agents! We only accept {} agents, but you have {} agents!"
    ...
    def _randomize_position_in_slot(self, target_vehicle_config):
        ...
        vehicle_config["spawn_longitude"] += self.np_random.uniform(
            -long / 2, long / 2)
        vehicle_config["spawn_lateral"] += self.np_random.uniform(
            -lat / 2, lat / 2)
