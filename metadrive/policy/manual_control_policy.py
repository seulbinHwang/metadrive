...
def get_controller(controller_name, pygame_control):
    ...
    if controller_name == "keyboard":
        return KeyboardController(pygame_control=pygame_control)
            ...
            "xboxController", "xboxcontroller", "xbox", "gamepad", "joystick",
        ...
        raise ValueError(
            "No such a controller type: {}".format(controller_name))
...
class ManualControlPolicy(EnvInputPolicy):
    ...
    def __init__(self, obj, seed, enable_expert=True):
        ...
        if config["manual_control"]:
            self.controller = get_controller(config["controller"],
                                             pygame_control=pygame_control)
            if self.controller is None:
                logger.warning(
                    "Load Joystick or Steering Wheel Error! Fall back to keyboard control"
                )
    ...
    def act(self, agent_id):
        ...
        try:
            ...
            print(
                "Current observation does not match the format that expert can accept."
            )
            self.toggle_takeover()

        is_track_vehicle = self.engine.agent_manager.get_agent(
            agent_id) is self.engine.current_track_agent
        not_in_native_bev = (self.engine.main_camera is None) or (
            not self.engine.main_camera.is_bird_view_camera())
        if self.engine.global_config[
                "manual_control"] and is_track_vehicle and not_in_native_bev:
            action = self.controller.process_input(
                self.engine.current_track_agent)
    ...
    def toggle_takeover(self):
        if self.engine.current_track_agent is not None:
            ...
            print("The expert takeover is set to: ",
                  self.engine.current_track_agent.expert_takeover)
...
class TakeoverPolicy(EnvInputPolicy):
    ...
    def act(self, agent_id):
        ...
        if self.engine.global_config[
                "manual_control"] and self.engine.agent_manager.get_agent(
                    agent_id
                ) is self.engine.current_track_agent and not self.engine.main_camera.is_bird_view_camera(
                ):
            expert_action = self.controller.process_input(
                self.engine.current_track_agent)
            if isinstance(self.controller, SteeringWheelController) and (
                    self.controller.left_shift_paddle or
                    self.controller.right_shift_paddle):
                ...
                return expert_action
                    ...
                    self.controller.takeover or abs(sum(expert_action)) > 0.01):
                self.takeover = True
                return expert_action
            elif isinstance(self.controller, XboxController) and (
                    self.controller.takeover or self.controller.button_a or
                    self.controller.button_b or self.controller.button_x or
                    self.controller.button_y or
                    abs(sum(expert_action)) > JOYSTICK_DEADZONE):
                self.takeover = True
                return expert_action
        self.takeover = False
        return agent_action
