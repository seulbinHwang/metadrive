...
class GPUFFT:
    ...
    def __init__(self, size, source_tex, normalization_factor):
        ...
        self.ping_texture.setup_2d_texture(self.size, self.size, Texture.TFloat,
                                           Texture.FRgba32)
        self.pong_texture = Texture("FFTPong")
        self.pong_texture.setup_2d_texture(self.size, self.size, Texture.TFloat,
                                           Texture.FRgba32)
        ...
        self.result_texture.setup2dTexture(self.size, self.size, Texture.TFloat,
                                           Texture.FRgba16)
    ...
    def _compute_weighting(self):
        ...
        indices_a = [
            [0 for i in range(self.size)] for k in range(self.log2_size)
        ]
        indices_b = [
            [0 for i in range(self.size)] for k in range(self.log2_size)
        ]
        weights = [
            [Vec2(0.0) for i in range(self.size)] for k in range(self.log2_size)
        ]
