...
from metadrive.render_pipeline.rpcore.water.water_manager import WaterManager
...
class ProjectedWater:
    ...
    def setup_water(self, pipeline, water_level):
        if (pipeline and self.model and isinstance(water_level, float) and
                self.water_options):

            self.pipeline = pipeline
            foam = Globals.base.loader.load_texture(
                "/$$rp/data/builtin_models/water/water_foam.png")
            ...
            self.model.set_shader_input("waterHeightfield",
                                        self.manager.get_displacement_texture())
            self.model.set_shader_input("waterNormal",
                                        self.manager.get_normal_texture())
            ...
            self.pipeline.set_effect(self.model,
                                     "/$$rp/effects/projected_water.yaml",
                                     {"render_shadow": False})
    ...
    def update_task(self, task):
        ...
        self.model.set_shader_input("cameraPosition",
                                    Globals.base.camera.get_pos(render))
