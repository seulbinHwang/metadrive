    ...
    def serialize_node(self, node, parent, index):
        ...
        if node in self.serialized_nodes:
            ...
            if isinstance(node, ScalarNode):
                ...
                implicit = (node.tag == self.resolve(SequenceNode, node.value,
                                                     True))
                self.emit(
                    SequenceStartEvent(alias,
                                       node.tag,
                                       implicit,
                ...
                implicit = (node.tag == self.resolve(MappingNode, node.value,
                                                     True))
                self.emit(
                    MappingStartEvent(alias,
                                      node.tag,
                                      implicit,
                                      flow_style=node.flow_style))
                for key, value in node.value:
