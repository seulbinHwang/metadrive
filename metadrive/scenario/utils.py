...
def draw_map(map_features, show=False):
    ...
    for key, value in map_features.items():
        if MetaDriveType.is_lane(value.get("type", None)):
            plt.scatter([x[0] for x in value["polyline"]],
                        [y[1] for y in value["polyline"]],
                        s=0.1)
...
def get_type_from_class(obj_class):
    ...
    elif issubclass(obj_class,
                    BaseTrafficLight) or obj_class is BaseTrafficLight:
...
def convert_recorded_scenario_exported(record_episode,
    ...
    result[SD.ID] = "{}-{}".format(record_episode["map_data"]["map_type"],
                                   record_episode["scenario_index"])
    ...
    if scenario_log_interval != 0.1:
        raise ValueError(
            "We don't support varying the scenario log interval yet.")

    frames = [
        step_frame_list[-1] for step_frame_list in record_episode["frame"]
    ]
    ...
    result[SD.METADATA][SD.SDC_ID] = str(
        frames[0]._agent_to_object[DEFAULT_AGENT])
    ...
    traffic_manager_name = find_traffic_manager_name(
        record_episode["manager_metadata"])
    light_manager_name = find_light_manager_name(
        record_episode["manager_metadata"])
    data_manager_name = find_data_manager_name(
        record_episode["manager_metadata"])
    ...
    if light_manager_name is not None:
        for frame in frames:
            all_lights.update(frame.manager_info[light_manager_name][
                SD.ORIGINAL_ID_TO_OBJ_ID].keys())
    ...
    lights = {
        k: {
            ...
            "state": {
                ScenarioDescription.TRAFFIC_LIGHT_STATUS:
                    [MetaDriveType.LIGHT_UNKNOWN] * episode_len
            },
            ScenarioDescription.TRAFFIC_LIGHT_POSITION:
                np.zeros(shape=(3,), dtype=np.float32),
            ScenarioDescription.TRAFFIC_LIGHT_LANE:
                None,
            "metadata":
                dict(track_length=episode_len,
    ...
    for frame_idx in range(result[SD.LENGTH]):
        ...
        for id, state in frames[frame_idx].step_info.items():
            ...
            if type == MetaDriveType.TRAFFIC_LIGHT:
                ...
                if id in tracks:
                    tracks.pop(id)
                assert light_manager_name in frames[
                    frame_idx].manager_info, "Can not find light manager info"

                # convert to original id
                id = frames[frame_idx].manager_info[light_manager_name][
                    SD.OBJ_ID_TO_ORIGINAL_ID][id]
                ...
                lights[id]["state"][ScenarioDescription.TRAFFIC_LIGHT_STATUS][
                    frame_idx] = light_status
                ...
                if lights[id][ScenarioDescription.TRAFFIC_LIGHT_LANE] is None:
                    ...
                    lights[id][
                        ScenarioDescription.TRAFFIC_LIGHT_POSITION] = state[
                            ScenarioDescription.TRAFFIC_LIGHT_POSITION]
                else:
                    assert lights[id][
                        ScenarioDescription.TRAFFIC_LIGHT_LANE] == str(id)
                    assert lights[id][
                        ScenarioDescription.TRAFFIC_LIGHT_POSITION] == state[
                            ScenarioDescription.TRAFFIC_LIGHT_POSITION]
                ...
                if "throttle_brake" in state:
                    if "throttle_brake" not in tracks[id]["state"]:
                        tracks[id]["state"]["throttle_brake"] = np.zeros(
                            shape=(episode_len, 1))
                    tracks[id]["state"]["throttle_brake"][frame_idx] = state[
                        "throttle_brake"]

                if "steering" in state:
                    if "steering" not in tracks[id]["state"]:
                        tracks[id]["state"]["steering"] = np.zeros(
                            shape=(episode_len, 1))
                    tracks[id]["state"]["steering"][frame_idx] = state[
                        "steering"]
                ...
                if id in frames[frame_idx]._object_to_agent:
                    tracks[id]["metadata"]["agent_name"] = frames[
                        frame_idx]._object_to_agent[id]

                if traffic_manager_name is not None:
                    origin_id = frames[frame_idx].manager_info[
                        traffic_manager_name][SD.OBJ_ID_TO_ORIGINAL_ID][id]
                    if tracks[id]["metadata"]["original_id"] == id:
                        tracks[id]["metadata"]["original_id"] = origin_id
                    else:
                        assert tracks[id]["metadata"][
            ...
            for key, policy_state in policy_info.items():
                ...
                if key not in tracks[id]["state"]:
                    tracks[id]["state"][key] = np.zeros(
                        shape=(episode_len, policy_state.size),
                        dtype=policy_state.dtype)
        ...
        for id, policy_spawn_info in frames[frame_idx].policy_spawn_info.items(
            ...
            tracks[id]["metadata"]["policy_spawn_info"] = copy.deepcopy(
                _convert_type_to_string(policy_spawn_info))
    ...
    result[SD.METADATA]["agent_to_object"] = {
        str(k): str(v) for k, v in agent_to_object.items()
    }
    result[SD.METADATA]["object_to_agent"] = {
        str(k): str(v) for k, v in object_to_agent.items()
    }

    if data_manager_name is not None:
        data_manager_raw_data = record_episode["manager_metadata"][
            data_manager_name].get("raw_data", None)
        if data_manager_raw_data:
            result[SD.METADATA]["history_metadata"] = data_manager_raw_data[
                "metadata"]
    if to_dict:
...
def read_scenario_data(file_path, centralize=False):
    ...
    """
    assert SD.is_scenario_file(
        file_path), "File: {} is not scenario file".format(file_path)
...
def read_dataset_summary(file_folder, check_file_existence=True):
    ...
    if os.path.isfile(summary_file):
        ...
        logger.warning(
            f"Summary file is not found at {summary_file}! Generate a dummy one."
        )
    ...
    if check_file_existence:
        for file in summary_dict:
            ...
            assert SD.is_scenario_file(
                file), "File:{} is not sd scenario file".format(file)
            file_path = file_folder / mapping[file] / file
            assert os.path.exists(file_path), "Can not find file: {}".format(
                file_path)

    return summary_dict, list(summary_dict.keys()), mapping


def extract_dataset_summary_and_mapping(scenario_list, dataset_name,
                                        dataset_version):
    ...
    for sd_scenario in scenario_list:
        ...
        export_file_name = SD.get_export_file_name(dataset_name,
        ...
        if export_file_name in summary:
            logger.warning(
                "Scenario {} already exists and will be overwritten!".format(
                    export_file_name))
...
def save_dataset(scenario_list, dataset_name, dataset_version, dataset_dir):
    summary, mapping, scenario_dict = extract_dataset_summary_and_mapping(
        scenario_list, dataset_name, dataset_version)
    summary_file = pathlib.Path(
        dataset_dir) / ScenarioDescription.DATASET.SUMMARY_FILE
    summary_file = summary_file.resolve()
    mapping_file = pathlib.Path(
        dataset_dir) / ScenarioDescription.DATASET.MAPPING_FILE
...
def assert_scenario_equal(scenarios1,
                          scenarios2,
                          only_compare_sdc=False,
    ...
    for scenario_id in scenarios1.keys():
        SD.sanity_check(scenarios1[scenario_id],
                        check_self_type=check_self_type)
        SD.sanity_check(scenarios2[scenario_id],
                        check_self_type=check_self_type)
        ...
        assert old_scene[SD.LENGTH] >= new_scene[SD.LENGTH], (
            old_scene[SD.LENGTH], new_scene[SD.LENGTH])
        ...
        if only_compare_sdc:
            ...
            min_len = min(state_dict1[SD.STATE]["position"].shape[0],
                          state_dict2[SD.STATE]["position"].shape[0])
            max_len = max(state_dict1[SD.STATE]["position"].shape[0],
                          state_dict2[SD.STATE]["position"].shape[0])
            ...
            for k in state_dict1[SD.STATE].keys():
                if k in ["action", "throttle_brake", "steering"]:
                    ...
                    np.testing.assert_almost_equal(
                        wrap_to_pi(state_dict1[SD.STATE][k][:min_len] -
            ...
            assert len(old_scene[SD.TRACKS]) == len(
                new_scene[SD.TRACKS]), "obj num mismatch"
            for track_id, track in old_scene[SD.TRACKS].items():
                ...
                for state_k in new_scene[SD.TRACKS][track_id][SD.STATE]:
                    state_array_1 = new_scene[SD.TRACKS][track_id][
                        SD.STATE][state_k]
                    state_array_2 = track[SD.STATE][state_k]
                    min_len = min(state_array_1.shape[0],
                                  state_array_2.shape[0])
                    max_len = max(state_array_1.shape[0],
                                  state_array_2.shape[0])
                    assert min_len / max_len > MIN_LENGTH_RATIO, "Replayed Scenario length ratio: {}".format(
                        min_len / max_len)
                    ...
                    if state_k == "heading":
                        ...
                        broader_ratio = 1
                        ret = abs(
                            wrap_to_pi(state_array_1[:min_len] -
                                       state_array_2[:min_len])) < 1e-1
                        ratio = np.sum(np.asarray(ret,
                                                  dtype=np.int8)) / len(ret)
                        if ratio < broader_ratio:
                            raise ValueError(
                                "Match ration: {}, Target: {}".format(
                                    ratio, broader_ratio))

                        strict_ratio = 0.98
                        ret = abs(
                            wrap_to_pi(state_array_1[:min_len] -
                                       state_array_2[:min_len])) < 1e-4
                        ratio = np.sum(np.asarray(ret,
                                                  dtype=np.int8)) / len(ret)
            ...
            for k in new_scene.get_sdc_track()["state"]:
                ...
                if k == "velocity":
                    ...
                    decimal = NP_ARRAY_DECIMAL
                np.testing.assert_almost_equal(state_array_1[:min_len],
                                               state_array_2[:min_len],
                                               decimal=decimal)

            assert new_scene[SD.TRACKS][track_id][SD.TYPE] == track[SD.TYPE]

        assert set(old_scene[SD.MAP_FEATURES].keys()).issuperset(
            set(new_scene[SD.MAP_FEATURES].keys()))
        assert set(old_scene[SD.DYNAMIC_MAP_STATES].keys()) == set(
            new_scene[SD.DYNAMIC_MAP_STATES].keys())
        ...
        for map_id, map_feat in old_scene[SD.MAP_FEATURES].items():
            ...
            if map_feat["type"] == MetaDriveType.LANE_SURFACE_STREET:
                assert len(map_feat["polyline"]) - len(
                    new_scene[SD.MAP_FEATURES][map_id]["polyline"]) <= 1
                _min = min(len(map_feat["polyline"]),
                           len(new_scene[SD.MAP_FEATURES][map_id]["polyline"]))
                np.testing.assert_almost_equal(
                    new_scene[SD.MAP_FEATURES][map_id]["polyline"][:_min],
                    map_feat["polyline"][:_min],
                    decimal=1)
            assert new_scene[SD.MAP_FEATURES][map_id][SD.TYPE] == map_feat[
                SD.TYPE]
        ...
        for obj_id, obj_state in old_scene[SD.DYNAMIC_MAP_STATES].items():
            ...
            for k in new_state_dict.keys():
                min_len = min(new_state_dict[k].shape[0],
                              old_state_dict[k].shape[0])
                max_len = max(new_state_dict[k].shape[0],
                              old_state_dict[k].shape[0])
                assert min_len / max_len > MIN_LENGTH_RATIO, "Replayed Scenario length ratio: {}".format(
                    min_len / max_len)
                if k == ScenarioDescription.TRAFFIC_LIGHT_STATUS:
                    same_light = new_state_dict[k][:min_len] == old_state_dict[
                        k][:min_len]
                    assert same_light.all()
                else:
                    np.testing.assert_almost_equal(new_state_dict[k][:min_len],
                                                   old_state_dict[k][:min_len],
                                                   decimal=NP_ARRAY_DECIMAL)

            assert new_scene[SD.DYNAMIC_MAP_STATES][obj_id][
                SD.TYPE] == obj_state[SD.TYPE]
