...
def _test_level(level=1, render=False):
    ...
    try:
        ...
        for i in tqdm(range(10), desc=str(level)):
            ...
            scenario_id.add(
                env.engine.data_manager.current_scenario_summary["id"])
        assert len(scenario_id) == (env.engine.current_level + 1) * int(
            10 / level)
    finally:
        env.close()
...
def test_curriculum_up_1_level(render=False, level=5):
    ...
    try:
        ...
        ids = [
            env.engine.data_manager.summary_dict[f]["id"]
            for f in env.engine.data_manager.summary_lookup
        ]
...
def _worker_env(render, worker_index, level_up=False):
    ...
    try:
        ...
        all_scenario = [
            env.engine.data_manager.summary_dict[f]["id"]
            for f in env.engine.data_manager.summary_lookup[:8]
        ]
...
def level_up_worker(render, worker_index):
    ...
    try:
        ...
        assert scenario_id[-5:] == [ids[-2] if worker_index == 0 else ids[-1]
                                   ] * 5 == scenario_id[-10:-5]
    finally:
        env.close()
    ...
    try:
        ...
        for i in tqdm(range(20), desc=str(2)):
            ...
            scenario_id.append(
                env.engine.data_manager.current_scenario_summary["id"])
        all_scenarios = sorted(list(
            env.engine.data_manager.summary_dict.keys()))[1:9]
