...
class DetectorMask:
    ...
    def __init__(self,
        ...
        self.masks = defaultdict(lambda: np.zeros(
            (self.num_lasers,), dtype=bool))
    ...
    def update_mask(self, position_dict: dict, heading_dict: dict,
                    is_target_vehicle_dict: dict):
        assert set(position_dict.keys()) == set(heading_dict.keys()) == set(
            is_target_vehicle_dict.keys())
        ...
        for c1, k1 in enumerate(keys[:-1]):
            for c2, k2 in enumerate(keys[c1 + 1:]):
                ...
                if (not is_target_vehicle_dict[k1]) and (
                        not is_target_vehicle_dict[k2]):
                    continue
                ...
                if is_target_vehicle_dict[k1]:
                    span = math.asin(
                        math.sqrt(self.half_max_span_square / dist_square))
    ...
    def get_mask(self, name):
        assert name in self.masks, "It seems that you have not initialized the mask for vehicle {} yet!".format(
            name)
...
def _search_angle(point1,
                  point2,
                  num_lasers,
                  start,
                  heading,
                  perceive_distance: float = 50):
    laser_heading = np.arange(0, num_lasers) * 2 * np.pi / num_lasers + heading
    result = []
    for laser_index in range(num_lasers):
        ret = _line_intersect(theta=laser_heading[laser_index],
                              center=start,
                              point1=point1,
                              point2=point2,
                              maximum=1e8)
...
def _test_mask(mask, stick_1_heading_deg, stick_2_heading_deg, max_span,
               stick1_x, stick2_x):
    ...
    mask.update_mask(position_dict={
        "stick1": stick1_pos,
        "stick2": stick2_pos
    },
                     heading_dict={
    ...
    real_mask_1 = _search_angle(point1=left_of_stick2,
                                point2=right_of_stick2,
                                num_lasers=360,
                                start=stick1_pos,
                                heading=stick_1_heading,
                                perceive_distance=100 * max_span) < 1.0
    res = np.stack([real_mask_1, mask_1])
    assert all(mask_1[real_mask_1]
              )  # mask 1 should at least include all True of real mask.
    ...
    real_mask_2 = _search_angle(point1=left_of_stick1,
            ...
            for pos2_x in pos_xy:
                ...
                for stick_1_heading_deg in angles1:
                    for stick_2_heading_deg in angles2:
                        _test_mask(mask, stick_1_heading_deg,
                                   stick_2_heading_deg, max_span, pos1_x,
                                   pos2_x)
...
def test_detector_mask_in_lidar():
    ...
    try:
        ...
        detector_mask = DetectorMask(
            ...
            span,
            max_distance=env.agent.config.lidar.distance)
        ep_count = 0
        for tt in range(3000):
            ...
            for v in env.engine.traffic_manager.vehicles:
                ...
                is_target_vehicle_dict[v.name] = True if isinstance(
                    v, BaseVehicle) else False
            ...
            if not all(mask[real_mask]):
                print('stop')
            assert all(mask[real_mask]
                      )  # mask 1 should at least include all True of real mask.

            print(
                "Num of true in our mask: {}, in old mask: {}. Overlap: {}. We have {} more."
                .format(sum(mask.astype(int)), sum(real_mask.astype(int)),
                        sum(mask[real_mask].astype(int)),
