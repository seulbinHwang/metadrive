...
def test_read_data_and_create_summary():
    ...
    sd_scenario = read_scenario_data(
        os.path.join(nuscenes, mapping[scenarios[2]], scenarios[2]))
    ...
    sd_scenario[SD.METADATA][SD.SUMMARY.NUMBER_SUMMARY] = SD.get_number_summary(
        sd_scenario)

    all_v = sd_scenario[SD.METADATA][SD.SUMMARY.NUMBER_SUMMARY][
        SD.SUMMARY.NUM_OBJECTS_EACH_TYPE][MetaDriveType.VEHICLE]
    moving_v = sd_scenario[SD.METADATA][SD.SUMMARY.NUMBER_SUMMARY][
        SD.SUMMARY.NUM_MOVING_OBJECTS_EACH_TYPE][MetaDriveType.VEHICLE]
...
def test_read_all_data_and_create_summary():
    ...
    for sid in range(len(scenarios)):
        sd_scenario = read_scenario_data(
            os.path.join(dataset_path, mapping[scenarios[sid]], scenarios[sid]))
        for track_id, track in sd_scenario[SD.TRACKS].items():
