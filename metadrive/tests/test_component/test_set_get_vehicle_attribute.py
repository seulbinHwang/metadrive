...
def test_set_get_vehicle_attribute(render=False):
    ...
    try:
        ...
        for _ in range(10):
            ...
            assert np.isclose(env.agent.velocity,
                              np.array([5, 0]),
                              rtol=1e-2,
                              atol=1e-2).all()
            assert abs(env.agent.speed - env.agent.speed_km_h / 3.6) < 1e-4
            assert np.isclose(env.agent.velocity,
                              env.agent.velocity_km_h / 3.6).all()
        ...
        for _ in range(10):
            ...
            assert np.isclose(env.agent.velocity,
                              np.array([
                                  5 / np.linalg.norm(np.array([5, 3])) * 10,
                                  3 / np.linalg.norm(np.array([5, 3])) * 10
                              ]),
                              rtol=1e-5,
                              atol=1e-5).all()
            assert abs(env.agent.speed - env.agent.speed_km_h / 3.6) < 1e-4
        ...
        for _ in range(10):
            ...
            assert np.isclose(
                env.agent.velocity,
                np.array([
                    0.3 / np.linalg.norm(np.array([0.3, 0.1])) * 10,
                    0.1 / np.linalg.norm(np.array([0.3, 0.1])) * 10
                ]),
                rtol=1e-5,
                atol=1e-5).all()
...
def test_coordinates(render=False):
    ...
    try:
        ...
        for _ in range(10):
            env.agent.set_velocity([5, 0], in_local_frame=True)
            o, r, tm, tc, info = env.step([0, 0])
        assert abs(env.agent.velocity[0] - 5.) < 1e-2 and abs(
            env.agent.velocity[1]) < 0.001
        ...
        for _ in range(10):
            o, r, tm, tc, info = env.step([0, 0])
        assert env.agent.velocity[0] < -3. and abs(
            env.agent.velocity[1]) < 0.001
        ...
        for _ in range(1):
            o, r, tm, tc, info = env.step([0, 0])
        assert env.agent.velocity[1] < -3. and abs(
            env.agent.velocity[0]) < 0.002
        ...
        assert env.agent.position[0] > begining_pos[0] and env.agent.position[
            1] > begining_pos[1]
        ...
        assert np.isclose(env.agent.heading,
                          np.array([np.sqrt(2) / 2,
                                    np.sqrt(2) / 2]), 1e-4, 1e-4).all()
        ...
        assert np.isclose(env.agent.heading, np.array([0, -1]), 1e-4,
                          1e-4).all()
    finally:
        env.close()
...
def test_set_angular_v_and_set_v_no_friction(render=False):
    ...
    try:
        ...
        for _ in range(100):
            ...
            o, r, tm, tc, info = env.step([0, 0])
        assert abs(wrap_to_pi(env.agent.heading_theta) -
                   np.pi) < 1e-2, env.agent.heading_theta
        ...
        for _ in range(100):
            ...
            o, r, tm, tc, info = env.step([0, 0])
        assert abs(env.agent.position[0] - start -
                   10) < 5e-2, env.agent.position
...
def test_set_angular_v_and_set_v_no_friction_pedestrian(render=False):
    ...
    try:
        ...
        obj_1 = env.engine.spawn_object(Pedestrian,
                                        position=[10, 3],
                                        heading_theta=0,
                                        random_seed=1)
        for _ in range(10):
            ...
            o, r, tm, tc, info = env.step([0, 0])
        assert abs(wrap_to_pi(obj_1.heading_theta) -
                   np.pi / 10) < 1e-2, obj_1.heading_theta
        obj_1.destroy()
