...
def test_conversion_functions():
    ...
    gymnasium_space_box = gymnasium.spaces.Box(0,
                                               1,
                                               shape=(1, 2, 3),
                                               dtype=np.float32)
    ...
    assert gymToGymnasium(
        gym_space_multi_discrete) == gymnasium_space_multi_discrete
    assert gymnasiumToGym(
        gymnasium_space_multi_discrete) == gym_space_multi_discrete

    gym_space_tuple = gym.spaces.Tuple([gym_space_box, gym_space_discrete])
    gymnasium_space_tuple = gymnasium.spaces.Tuple(
        [gymnasium_space_box, gymnasium_space_discrete])
    assert gymToGymnasium(gym_space_tuple) == gymnasium_space_tuple
    assert gymnasiumToGym(gymnasium_space_tuple) == gym_space_tuple

    gym_space_dict = gym.spaces.Dict({
        "a": gym_space_box,
        "b": gym_space_discrete
    })
    gymnasium_space_dict = gymnasium.spaces.Dict({
        "a": gymnasium_space_box,
        "b": gymnasium_space_discrete
    })
...
class NoModeGymnasiumInnerClass(gymnasium.Env):
    ...
    def __init__(self, config: dict):
        ...
        self.observation_space = gymnasium.spaces.Box(0,
                                                      1,
                                                      shape=(1, 2, 3),
                                                      dtype=np.float32)
...
def test_config_override():
    ...
    try:
        ...
        raise AssertionError(
            "OverridenGymWrapperedClass_1 should have raised an AssertionError")
...
class SingleAgentEnvironment(gymnasium.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        if self.steps < 50:
            ...
            if action[0] < 0:
                self.active = False
                return self.observation_space.sample(), float(
                    self.reward_source.sample().item()), True, False, {}
            else:
                return self.observation_space.sample(), float(
                    self.reward_source.sample().item()), False, False, {}
        else:
            self.active = False
            return self.observation_space.sample(), float(
                self.reward_source.sample().item()), False, True, {}
...
def test_gym_wrapper_single_agent():
    ...
    assert gymToGymnasium(
        gym_env.observation_space) == gymnasium_env.observation_space
    ...
    while True:
        ...
        o_gym, r_gym, d_gym, _ = gym_env.step(
            np.array([0.0, 0.0], dtype=np.float32))
...
class MultiAgentEnvironment(gymnasium.Env):
    ...
    def _generate_rewards(self) -> Dict[str, float]:
        return {
            agent: float(self.reward_source.sample().item())
            for agent in sorted(self.agents)
        }
    ...
    def _validate_actions(self, actions: Dict[str, np.ndarray]) -> bool:
        if set(actions.keys()) != self.agents:
            return False
        return all(
            self.action_space.contains(action) for action in actions.values())
    ...
    def step(self, action: Dict[str, np.ndarray]):
        ...
        truncateds = {
            agent: self.steps >= self.horizon and not terminateds[agent]
        ...
        self.agents = {
            ...
            if not terminateds[agent] and not truncateds[agent]
        }
...
def test_gym_wrapper_multi_agent():
    ...
    gymnasium_env = MultiAgentEnvironment(config={
        "num_agents": 5,
        "horizon": 50
    })
    gym_env = createGymWrapper(MultiAgentEnvironment)(config={
        "num_agents": 5,
        "horizon": 50
    })
    ...
    while True:
        ...
        a_dict = {
            agent: np.array([0.0, 0.0], dtype=np.float32)
            for agent in gymnasium_env.agents
        }
    ...
    while True:
        ...
        a_dict[sorted(a_dict.keys())[-1]] = np.array([-0.5, 0],
                                                     dtype=np.float32)
