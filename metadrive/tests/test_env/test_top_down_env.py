...
def test_top_down_rendering():
    for env in [
            TopDownSingleFrameMetaDriveEnv(
                dict(num_scenarios=5, map="C", traffic_density=1.0)),
            TopDownMetaDrive(dict(num_scenarios=5, map="C",
                                  traffic_density=1.0)),
            TopDownMetaDrive(
                dict(num_scenarios=5, map="C", frame_stack=1, post_stack=2)),
            TopDownMetaDriveEnvV2(
                dict(num_scenarios=5, map="C", frame_stack=1, post_stack=2)),
            ScenarioEnv(
                dict(
                    num_scenarios=1,
