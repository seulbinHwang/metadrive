        ...
        for i in range(1, 100000 if vis else 2000):
            ...
            if tm or tc:
                epi_info = env.engine.dump_episode(
                    "test_dump_single.pkl" if test_dump else None)
                break
        ...
        for i in range(0, 100000 if vis else 2000):
            ...
            for old_id, new_id in env.engine.replay_manager.record_name_to_current_name.items(
                ...
                record_pos = env.engine.replay_manager.current_frame.step_info[
                    old_id]["position"]
                record_heading = env.engine.replay_manager.current_frame.step_info[
                    old_id]["heading_theta"]
                assert np.isclose(np.array([pos[0], pos[1],
                                            obj.get_z()]),
                                  np.array(record_pos)).all()
                assert abs(wrap_to_pi(heading - record_heading)) < 1e-2

                assert np.isclose(np.array([pos[0], pos[1]]),
                                  np.array(step_info[i][old_id][0]), 1e-2,
                                  1e-2).all()
                assert abs(
                    wrap_to_pi(heading -
                               np.array(step_info[i][old_id][1]))) < 1e-2
    ...
    try:
        ...
        for i in range(1, 100000 if vis else 600):
            o, r, tm, tc, info = env.step(
                {agent_id: [0, .2] for agent_id in env.agents.keys()})
            if vis:
                env.render()
            if tm["__all__"]:
        ...
        for i in range(1, 100000 if vis else 2000):
            ...
            o, r, tm, tc, info = env.step(
                {agent_id: [0, 0.1] for agent_id in env.agents.keys()})
            if vis:
                env.render()
            if tm["__all__"]:
                break
