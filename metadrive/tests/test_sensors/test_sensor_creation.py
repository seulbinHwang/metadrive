...
def test_creation():
    ...
    assert env.config["sensors"].keys() == {
        "lidar", "side_detector", "lane_line_detector", "semantic", "rgb",
        "main_camera", "dashboard"
    ...
    env = MetaDriveEnv({
        ...
        "interface_panel": ["rgb", "dashboard"],
        "image_observation":
            False,  # it is a switch telling metadrive to use rgb as observation
    })
    env.reset()
    assert env.config["sensors"].keys() == {
        "lidar", "side_detector", "lane_line_detector"
    } == env.engine.sensors.keys()
