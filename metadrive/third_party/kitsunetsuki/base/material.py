    ...
    for link in node_tree.links:
        ...
        if (link.to_node == to_node and link.to_socket.name == to_socket_name):
            ...
            # Mix Shader [Shader] ->
                ...
                for input_ in mix.inputs:
                    if input_.name == 'Shader':
                        node = get_from_node(node_tree, type, mix,
                                             from_socket_name, 'Shader')
                        if node:
                            return node
            ...
            # Math [Value] ->
                ...
                for input_ in math.inputs:
                    if input_.name == 'Value':
                        node = get_from_node(node_tree, type, math,
            ...
            # Separate RGB [Value] ->
                ...
                node = get_from_node(node_tree, type, seprgb, from_socket_name,
                                     'Image')
                if node:
                    return node
