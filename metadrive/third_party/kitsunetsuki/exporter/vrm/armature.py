...
class ArmatureMixin(object):
    ...
    def _make_vrm_bone(self, gltf_node_id, bone):
        ...
        if is_hips(bone):
            vrm_bone['bone'] = 'hips'
              ...
              (is_bone_matches(bone,
                               ('spine',)) and is_hips(get_parent(bone, 3)))):
        ...
        elif is_bone_matches(bone,
                             ('lowerarm', 'lower_arm', 'forearm', 'elbow')):
    ...
    def _make_vrm_spring(self, gltf_node_id, bone):
        vrm_spring = {
            'comment': bone.name,
            'stiffiness':
                1,  # The resilience of the swaying object (the power of returning to the initial pose)
            'gravityPower': 0,
            'dragForce':
                0,  # The resistance (deceleration) of automatic animation
            'gravityDir': {
                'x': 0,
                'y': -1,
                'z': 0,
    ...
    def make_armature(self, parent_node, armature):
        ...
        for bone_name, bone in armature.data.bones.items():
            ...
            if vrm_bone['bone'] and vrm_bone['bone'] not in vrm_bones:
                ...
                self._root['extensions']['VRM']['humanoid'][
                    'humanBones'].append(vrm_bone)
                ...
                if vrm_bone['bone'] == 'head':
                    ...
                    for c in pose_bone.constraints:
                        if c.type == 'LIMIT_ROTATION':
                            fp['lookAtHorizontalOuter'][
                                'xRange'] = -math.degrees(c.min_x)
                            fp['lookAtHorizontalInner'][
                                'xRange'] = math.degrees(c.max_x)
                            fp['lookAtVerticalDown']['yRange'] = -math.degrees(
                                c.min_z)
                            fp['lookAtVerticalUp']['yRange'] = math.degrees(
                                c.max_z)
                            break
            ...
            if pose_bone.get('jiggle_enable', False):
                ...
                if pose_bone.name not in vrm_springs:
                    ...
                    self._root['extensions']['VRM']['secondaryAnimation'][
                        'boneGroups'].append(vrm_spring)
