...
class AgentTemporalState:
    ...
    def previous_state(self) -> Optional[Waypoint]:
        ...
        if not self.past_trajectory or len(
                self.past_trajectory.valid_waypoints) < 2:
            return None
    ...
    def predictions(self,
                    predicted_trajectories: List[PredictedTrajectory]) -> None:
        """
        ...
        probability_sum = sum(
            prediction.probability for prediction in predicted_trajectories)
        if not abs(probability_sum - 1) < 1e-6 and predicted_trajectories:
            raise ValueError(
                f"The provided trajectory probabilities did not sum to one, but to {probability_sum:.2f}!"
            )
        self._predictions = predicted_trajectories
    ...
    def past_trajectory(self,
        ...
        if not last_waypoint:
            raise RuntimeError(
                "Last waypoint represents current agent's state, this should not be None!"
            )
