...
class OrientedBox:
    ...
    def dimensions(self) -> Dimension:
        ...
        """
        return Dimension(length=self.length,
                         width=self.width,
                         height=self.height)
    ...
    def corner(self, point: OrientedBoxPointType) -> Point2D:
        ...
        if point == OrientedBoxPointType.FRONT_LEFT:
            return translate_longitudinally_and_laterally(
                self.center, self.half_length, self.half_width).point
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (math.isclose(self.width, other.width) and
                math.isclose(self.height, other.height) and
...
def collision_by_radius_check(box1: OrientedBox, box2: OrientedBox,
                              radius_threshold: Optional[float]) -> bool:
    """
...
def in_collision(box1: OrientedBox,
                 box2: OrientedBox,
                 radius_threshold: Optional[float] = None) -> bool:
    """
    ...
    """
    return (bool(box1.geometry.intersects(box2.geometry)) if
            collision_by_radius_check(box1, box2, radius_threshold) else False)
