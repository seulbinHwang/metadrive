...
class SceneObject:
    ...
    def __init__(self, tracked_object_type: TrackedObjectType,
                 oriented_box: OrientedBox, metadata: SceneObjectMetadata):
        """
    ...
    @classmethod
    def make_random(cls, token: str,
                    object_type: TrackedObjectType) -> SceneObject:
        """
        ...
        return SceneObject(
            metadata=SceneObjectMetadata(token=token,
                                         track_id=track_id,
                                         track_token=token,
                                         timestamp_us=timestamp_us),
            tracked_object_type=object_type,
            oriented_box=OrientedBox(StateSE2(*center, heading), size[0],
                                     size[1], size[2]),
        )
