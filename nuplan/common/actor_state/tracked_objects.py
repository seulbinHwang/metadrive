...
class TrackedObjects:
    ...
    def __init__(self, tracked_objects: Optional[List[TrackedObject]] = None):
        ...
        self.tracked_objects = sorted(
            tracked_objects,
            key=lambda agent: agent.tracked_object_type.value  # type: ignore
        )
    ...
    def from_oriented_boxes(cls, boxes: List[OrientedBox]) -> TrackedObjects:
        ...
        scene_objects = [
            SceneObject(
                ...
                box,
                SceneObjectMetadata(timestamp_us=i,
                                    token=str(i),
                                    track_token=None,
                                    track_id=None),
    ...
    def get_tracked_objects_of_type(
            self,
            tracked_object_type: TrackedObjectType) -> List[TrackedObject]:
        """
    ...
    def get_static_objects(self) -> List[StaticObject]:
        ...
        for static_object_type in STATIC_OBJECT_TYPES:
            static_objects.extend(
                self.get_tracked_objects_of_type(static_object_type))
    ...
    def get_tracked_objects_of_types(
        ...
        for _type in tracked_object_types:
            open_loop_tracked_objects.extend(
                self.get_tracked_objects_of_type(_type))
