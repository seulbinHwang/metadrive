...
class Waypoint(InterpolatableState):
    ...
    def __init__(self,
                 ...
                 velocity: Optional[StateVector2D] = None):
        """
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return (other.oriented_box == self._oriented_box and
                other.time_point == self.time_point and
                other.velocity == self._velocity)
    ...
    def deserialize(vector: List[Union[int, float]]) -> Waypoint:
        ...
        """
        assert len(
            vector) == 9, f'Expected a vector of size 9, got {len(vector)}'

        return Waypoint(
            time_point=TimePoint(int(vector[0])),
            oriented_box=OrientedBox(StateSE2(vector[1], vector[2], vector[3]),
                                     vector[4], vector[5], vector[6]),
            velocity=StateVector2D(vector[7], vector[8])
            if vector[7] is not None and vector[8] is not None else None,
        )
    ...
    def to_split_state(self) -> SplitState:
        ...
        fixed_state = [
            self._oriented_box.width, self._oriented_box.length,
            self._oriented_box.height
        ]
    ...
    def from_split_state(split_state: SplitState) -> Waypoint:
        ...
        return Waypoint(
            ...
            oriented_box=OrientedBox(
                StateSE2(split_state.linear_states[1],
                         split_state.linear_states[2],
                         split_state.angular_states[0]),
                length=split_state.fixed_states[1],
                width=split_state.fixed_states[0],
                height=split_state.fixed_states[2],
            ),
            velocity=StateVector2D(split_state.linear_states[3],
                                   split_state.linear_states[4])
            if split_state.linear_states[3] is not None and
            split_state.linear_states[4] is not None else None,
        )
