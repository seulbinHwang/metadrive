...
def lateral_distance(reference: StateSE2, other: Point2D) -> float:
    ...
    """
    return float(-np.sin(reference.heading) * (other.x - reference.x) +
                 np.cos(reference.heading) * (other.y - reference.y))
...
def longitudinal_distance(reference: StateSE2, other: Point2D) -> float:
    ...
    """
    return float(
        np.cos(reference.heading) * (other.x - reference.x) +
        np.sin(reference.heading) * (other.y - reference.y))
...
def signed_lateral_distance(ego_state: StateSE2, other: Polygon) -> float:
    ...
    distance_left = max(
        min(
            lateral_distance(ego_left, Point2D(*vertex))
            for vertex in vertices), 0)
    distance_right = max(
        min(-lateral_distance(ego_right, Point2D(*vertex))
            for vertex in vertices), 0)
...
def signed_longitudinal_distance(ego_state: StateSE2, other: Polygon) -> float:
    ...
    distance_front = max(
        min(
            longitudinal_distance(ego_front, Point2D(*vertex))
            for vertex in vertices), 0)
    distance_back = max(
        min(-longitudinal_distance(ego_back, Point2D(*vertex))
            for vertex in vertices), 0)
...
def principal_value(
        angle: Union[float, int, npt.NDArray[np.float64]],
        min_: float = -np.pi) -> Union[float, npt.NDArray[np.float64]]:
    """
...
def l2_euclidean_corners_distance(box1: OrientedBox,
                                  box2: OrientedBox) -> float:
    """
    ...
    distances = [
        ...
        for box1_corner, box2_corner in zip(box1.all_corners(),
                                            box2.all_corners())
    ]
    return float(np.linalg.norm(distances))


def se2_box_distances(query: StateSE2,
                      targets: list[StateSE2],
                      box_size: Dimension,
                      consider_flipped: bool = True) -> List[float]:
    """
    ...
    """
    query_box = OrientedBox(query, box_size.length, box_size.width,
                            box_size.height)
    backwards_query_box = OrientedBox.from_new_pose(
        query_box, StateSE2(query.x, query.y, query.heading + np.pi))
    target_boxes = [
        OrientedBox(target, box_size.length, box_size.width, box_size.height)
