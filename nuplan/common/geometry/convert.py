...
def matrix_from_pose(pose: StateSE2) -> npt.NDArray[np.float64]:
    ...
    return np.array([
        ...
        [np.sin(pose.heading),
         np.cos(pose.heading), pose.y],
        [0, 0, 1],
    ])


def absolute_to_relative_poses(
        absolute_poses: List[StateSE2]) -> List[StateSE2]:
    """
    ...
    """
    absolute_transforms: npt.NDArray[np.float64] = np.array(
        [matrix_from_pose(pose) for pose in absolute_poses])
    origin_transform = np.linalg.inv(absolute_transforms[0])
    relative_transforms = origin_transform @ absolute_transforms
    relative_poses = [
        pose_from_matrix(transform_matrix)
        for transform_matrix in relative_transforms
    ]
...
def relative_to_absolute_poses(
        origin_pose: StateSE2,
        relative_poses: List[StateSE2]) -> List[StateSE2]:
    """
    ...
    absolute_transforms: npt.NDArray[
        np.float32] = origin_transform @ relative_transforms
    absolute_poses = [
        pose_from_matrix(transform_matrix)
        for transform_matrix in absolute_transforms
    ]
...
def numpy_array_to_absolute_velocity(
        ...
        velocities: npt.NDArray[np.float32]) -> List[StateVector2D]:
    """
    ...
    assert velocities.shape[
        1] == 2, f"Expected poses shape of (*, 2), got {velocities.shape}"
    velocities = np.pad(velocities.astype(np.float64), ((0, 0), (0, 1)),
                        "constant",
                        constant_values=0.0)
    relative_states = [StateSE2.deserialize(pose) for pose in velocities]
    return [
        StateVector2D(state.x, state.y) for state in relative_to_absolute_poses(
            origin_absolute_state, relative_states)
    ]


def numpy_array_to_absolute_pose(
        origin_absolute_state: StateSE2,
        poses: npt.NDArray[np.float32]) -> List[StateSE2]:
    """
    ...
    """
    assert poses.shape[
        1] == 3, f"Expected poses shape of (*, 3), got {poses.shape}"
...
def vector_2d_from_magnitude_angle(magnitude: float,
                                   angle: float) -> StateVector2D:
    """
