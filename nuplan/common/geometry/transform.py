...
def rotate_2d(point: Point2D,
              rotation_matrix: npt.NDArray[np.float64]) -> Point2D:
    """
...
def translate(pose: StateSE2, translation: npt.NDArray[np.float64]) -> StateSE2:
    ...
    return StateSE2(pose.x + translation[0], pose.y + translation[1],
                    pose.heading)


def rotate(pose: StateSE2,
           rotation_matrix: npt.NDArray[np.float64]) -> StateSE2:
    """
    ...
    return StateSE2(rotated_point[0], rotated_point[1],
                    pose.heading + rotation_angle)
...
def transform(pose: StateSE2,
              transform_matrix: npt.NDArray[np.float64]) -> StateSE2:
    """
...
def translate_longitudinally(pose: StateSE2, distance: float) -> StateSE2:
    ...
    translation: npt.NDArray[np.float64] = np.array(
        [distance * np.cos(pose.heading), distance * np.sin(pose.heading)])
...
def translate_laterally(pose: StateSE2, distance: float) -> StateSE2:
    ...
    half_pi = np.pi / 2.0
    translation: npt.NDArray[np.float64] = np.array([
        distance * np.cos(pose.heading + half_pi),
        distance * np.sin(pose.heading + half_pi)
    ])
    return translate(pose, translation)


def translate_longitudinally_and_laterally(pose: StateSE2, lon: float,
                                           lat: float) -> StateSE2:
    """
