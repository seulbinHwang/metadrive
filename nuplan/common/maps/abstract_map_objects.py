...
class LaneGraphEdgeMapObject(GraphEdgeMapObject):
    ...
    def is_adjacent_to(self, other: Lane) -> bool:
        ...
        return self.is_same_roadblock(other) and (
            self.right_boundary.id == other.left_boundary.id or
            self.left_boundary.id == other.right_boundary.id)
    ...
    def adjacent_edges(
        self
    ) -> Tuple[Optional[LaneGraphEdgeMapObject],
    ...
    def get_width_left_right(
            self,
            point: Point2D,
            include_outside: bool = False) -> Tuple[float, float]:
        """
...
class PolylineMapObject(AbstractMapObject):
    ...
    def get_nearest_curvature_from_position(self, point: Point2D) -> float:
        ...
        :return: [1/m] curvature along a polyline.
        """
        return self.get_curvature_at_arc_length(
            self.get_nearest_arc_length_from_position(point))
...
class RoadBlockGraphEdgeMapObject(GraphEdgeMapObject):
    ...
    @abc.abstractmethod
    def incoming_edges(
            self) -> List[RoadBlockGraphEdgeMapObject]:  # type: ignore
        """
        Returns incoming edges connecting to this edge.
