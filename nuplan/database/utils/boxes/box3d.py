...
def points_in_box(box: Box3D,
                  points: npt.NDArray[np.float64],
                  wlh_factor: float = 1.0) -> npt.NDArray[np.float64]:
    """
    ...
    pts_mask = functools.reduce(np.logical_and, [
        x >= cx - r, x <= cx + r, y >= cy - r, y <= cy + r, z >= cz - r, z
        <= cz + r
    ])
...
def points_in_box_bev(
        box: Box3D, points: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    """
...
class Box3D(BoxInterface):
    ...
    _min_size = np.finfo(
        np.float32
    ).eps  # type: ignore  # This is the smallest size within the float32 precision.
    ...
    def __init__(
        ...
        assert size[
            0] > self._min_size, "Error: box Width must be larger than {} cm".format(
                100 * self._min_size)
        assert size[
            1] > self._min_size, "Error: box Length must be larger than {} cm".format(
                100 * self._min_size)
        assert size[
            2] > self._min_size, "Error: box Height must be larger than {} cm".format(
                100 * self._min_size)
        ...
        self.angular_velocity = float(angular_velocity) if not np.isnan(
            angular_velocity) else angular_velocity
    ...
    def distance_3d(self) -> float:
        ...
        """
        return float(
            (self.center[0]**2 + self.center[1]**2 + self.center[2]**2)**0.5)
    ...
    def init_trajectory_fields(
        ...
        if not self.mode_probs.shape[0] == self.future_centers.shape[0] == len(
                self.future_orientations):  # type: ignore
            raise ValueError(
        ...
        if self.future_centers.shape[1] != len(
                self.future_orientations[0]):  # type: ignore
            raise ValueError(
    ...
    def _set_color(self) -> None:
        ...
        if self._labelmap is None or self.label not in self._labelmap:
            if self.label is None or np.isnan(self.label):
                self._color = (
                    255, 61, 99, 0
                )  # type: ignore  # Set color to Red when label is not set.
            else:
    ...
    def equate_orientations(self, other: object) -> bool:
        ...
        """
        if (self.future_orientations is None) != (other.future_orientations
                                                  is None):  # type: ignore
            return False
        if self.future_orientations is not None and other.future_orientations is not None:  # type: ignore
            for mode_idx in range(self.num_modes):  # type: ignore
                for horizon_idx in range(
                        self.num_future_timesteps):  # type: ignore
                    self_future_orientation = self.future_orientations[
                        mode_idx][horizon_idx]
                    other_future_orientation = other.future_orientations[
                        mode_idx][horizon_idx]  # type: ignore
                    if (self_future_orientation
                            is None) != (other_future_orientation is None):
                        return False
                    if self_future_orientation is not None and other_future_orientation is not None:
                        if not np.allclose(
                                self.future_orientations[mode_idx]
                            [horizon_idx].rotation_matrix,
                                other.future_orientations[mode_idx]
                            [horizon_idx].rotation_matrix,  # type: ignore
                                atol=1e-04,
                        ):
                            return False
        return True
    ...
    def __eq__(self, other: object) -> bool:
        ...
        orientation = np.allclose(self.orientation.rotation_matrix,
                                  other.orientation.rotation_matrix,
                                  atol=1e-04)
        label = (self.label == other.label) or (np.isnan(self.label) and
                                                np.isnan(other.label))
        score = (self.score == other.score) or (np.isnan(self.score) and
                                                np.isnan(other.score))
        vel = np.allclose(self.velocity, other.velocity,
                          atol=1e-04) or (np.all(np.isnan(self.velocity)) and
                                          np.all(np.isnan(other.velocity)))
        angular_vel = np.isclose(
            self.angular_velocity, other.angular_velocity,
            atol=1e-04) or (np.isnan(self.angular_velocity) and
                            np.isnan(other.angular_velocity))
        payload = self.payload == other.payload

        if not (center and wlh and orientation and label and score and vel and
                angular_vel and payload):
            return False
        ...
        if self.future_centers is not None and other.future_centers is not None:
            if not np.array_equal(np.isnan(self.future_centers),
                                  np.isnan(other.future_centers)):
                return False
            if not np.allclose(
    ...
    def __repr__(self) -> str:
        ...
        if self.future_horizon_len_s is not None:
            arguments += ', future_horizon_len_s=\'{}\''.format(
                self.future_horizon_len_s)
        if self.future_interval_s is not None:
            arguments += ', future_interval_s=\'{}\''.format(
        ...
        if self.future_orientations is not None:
            arguments += ', future_orientations=\'{}\''.format(
                self.future_orientations)
    ...
    def serialize(self) -> Dict[str, Any]:
        ...
        ] if self.future_orientations is not None else None)
        ...
        return {
            ...
            'future_interval_s':
                self.future_interval_s,
            'future_centers':
                self.future_centers.tolist()
                if self.future_centers is not None else None,  # type: ignore
            'future_orientations':
                future_orientations_serialized,
            'mode_probs':
                self.mode_probs.tolist()
                if self.mode_probs is not None else None,  # type: ignore
        }
    ...
    def deserialize(cls, data: Dict[str, Any]) -> Box3D:
        ...
        if type(data) is dict:
            ...
            ] if data['future_orientations'] is not None else None)
            return Box3D(
                data['center'],
                data['wlh'],
            ...
            for mode_idx in range(self.num_modes):
                for horizon_idx in range(self.num_future_timesteps):
                    self.future_centers[mode_idx][horizon_idx] = np.dot(
                        rotation_matrix,
                        self.future_centers[mode_idx][horizon_idx])
    ...
    def xflip(self) -> None:
        ...
        if self.future_orientations is not None:
            for mode_idx in range(self.num_modes):  # type: ignore
                for horizon_idx in range(
                        self.num_future_timesteps):  # type: ignore
                    orientation = self.future_orientations[mode_idx][
                        horizon_idx]
                    ...
                    final_yaw = -current_yaw + np.pi
                    self.future_orientations[mode_idx][
                        horizon_idx] = Quaternion(axis=(0, 0, 1),
                                                  angle=final_yaw)
    ...
    def velocity_endpoint(self) -> npt.NDArray[np.float64]:
        ...
        :return: <np.float: 3, 1>.
        """
        return self.center_bottom_forward + np.expand_dims(self.velocity.T,
                                                           axis=1)
    ...
    def get_future_horizon_idx(self, future_horizon_s: float) -> int:
        ...
        if not 0.0 < future_horizon_s <= self.future_horizon_len_s:
            raise ValueError(
                f"Future horizon ({future_horizon_s}) should be in (0, {self.future_horizon_len_s}]."
            )
    ...
    def get_future_center_at_horizon(
            self, future_horizon_s: float) -> npt.NDArray[np.float64]:
        """
    ...
    def get_future_centers_at_horizons(
            self, future_horizons_s: List[float]) -> npt.NDArray[np.float64]:
        """
        ...
        highest_prob_mode_idx = self.get_highest_prob_mode_idx()
        horizon_indices = [
            self.get_future_horizon_idx(future_horizon_s)
            for future_horizon_s in future_horizons_s
        ]
        return self.future_centers[highest_prob_mode_idx, horizon_indices]

    def get_future_orientation_at_horizon(
            self, future_horizon_s: float) -> Quaternion:
        """
    ...
    def get_future_orientations_at_horizons(
            self, future_horizons_s: List[float]) -> List[Quaternion]:
        """
        ...
        return [
            self.future_orientations[highest_prob_mode_idx][horizon_idx]
            for horizon_idx in horizon_indices
        ]

    def get_topk_future_center_at_horizon(self, future_horizon_s: float,
                                          topk: int) -> npt.NDArray[np.float64]:
        """
    ...
    def get_topk_future_orientation_at_horizon(self, future_horizon_s: float,
                                               topk: int) -> List[Quaternion]:
        """
        ...
        return [
            self.future_orientations[mode_idx][horizon_idx]
            for mode_idx in topk_mode_indices
        ]
    ...
    def draw_line(
        ...
        if isinstance(canvas, np.ndarray):
            ...
            cv2.line(canvas, (int(from_x), int(from_y)), (int(to_x), int(to_y)),
                     color_int[::-1], linewidth)
        else:
            canvas.plot([from_x, to_x], [from_y, to_y],
                        color=color,
                        linewidth=linewidth,
                        marker=marker,
                        alpha=alpha)
    ...
    def draw_text(self, canvas: Union[plt.Axes, npt.NDArray[np.uint8]],
                  x: float, y: float, text: str) -> None:
        """
    ...
    def render(
        ...
        colors: Tuple[MatplotlibColor, MatplotlibColor,
                      MatplotlibColor] = None,  # type: ignore
        linewidth: float = 2,
        marker: str = 'o',
        ...
        colors = tuple(
            matplotlib.colors.to_rgb(c) if isinstance(c, str) else c
        ...
        self.draw_rect(canvas, corners.T[:4], colors[0],
                       linewidth)  # type: ignore
        self.draw_rect(canvas, corners.T[4:], colors[1],
                       linewidth)  # type: ignore
        ...
        if with_velocity and not any(np.isnan(self.velocity)):
            center_bottom_forward = np.mean(corners.T[2:4], axis=0)
            velocity_end = view_points(self.velocity_endpoint,
                                       view,
        ...
        if with_label:
            ...
            proj_center = view_points(org_center, view, normalize=normalize)[:2,
                                                                             0]
            self.draw_text(canvas, proj_center[0], proj_center[1],
                           str(self.label))
            ...
            for mode_idx in range(self.num_modes):
                ...
                for horizon_idx in range(self.num_future_timesteps):
                    if self.num_future_timesteps > 1:
                        ...
                        color = self.fade_color(color_int, horizon_idx,
    ...
    def fade_color(color: Tuple[int, int, int], step: int,
        ...
        increment = (float(hsv_color[2]) / 255.0 -
                     LOWEST_VALUE) / total_number_of_steps

        new_value = float(hsv_color[2]) / 255.0 - step * increment

        new_rgb = colorsys.hsv_to_rgb(float(hsv_color[0]), float(hsv_color[1]),
                                      new_value * 255.0)
    ...
    @staticmethod
    @functools.lru_cache()
    def _calc_corners(width: float, length: float, height: float,
                      center: Tuple[float],
                      rotation_matrix: Tuple[float]) -> npt.NDArray[np.float64]:
        """
