...
def union(a: Rectangle, b: Rectangle) -> float:
    ...
    """
    return (a[2] - a[0]) * (a[3] - a[1]) + (b[2] - b[0]) * (
        b[3] - b[1]) - intersection(a, b)


def birdview_corner_angle_mean_distance(a: TwoDimBox, b: TwoDimBox,
                                        period: float) -> float:
    """
    ...
    avg_abs_error = float(
        np.mean(np.abs(np.concatenate((box_error, np.array([yaw_error]))))))

    return avg_abs_error


def birdview_corner_angle_mean_distance_box(a: Box3D, b: Box3D,
                                            period: float) -> float:
    """
...
def angle_diff_numpy(x: npt.NDArray[np.float64], y: npt.NDArray[np.float64],
                     period: float) -> npt.NDArray[np.float64]:
    """
...
def hausdorff_distance_box(obsbox: Box3D, gtbox: Box3D) -> float:
    ...
    def footprint(box: Box3D) -> Polygon:
        ...
        """
        x, y, w, l, head = (box.center[0], box.center[1], box.wlh[0],
                            box.wlh[1], quaternion_yaw(box.orientation))
        rot = np.array([[math.cos(head), -math.sin(head)],
                        [math.sin(head), math.cos(head)]])  # type: ignore
        ...
        return Polygon([(q1.item(0), q1.item(1)), (q2.item(0), q2.item(1)),
                        (q3.item(0), q3.item(1)), (q4.item(0), q4.item(1))])
...
def birdview_center_distance_box(a: Box3D, b: Box3D) -> float:
    ...
    """
    return float(
        np.sqrt((a.center[0] - b.center[0])**2 +
...
def assign(box_list1: List[Any], box_list2: List[Any],
    ...
    pairs_list = [
        pair for pair in pairs if costmatrix[pair[0], pair[1]] < assign_th
    ]
...
def long_lat_dist_decomposition(
        gt_vector: npt.NDArray[np.float64],
