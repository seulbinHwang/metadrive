...
def _color_prep(
    ...
    if colors is None:
        ...
        if ncolors is not None:
            assert ncolors == len(
                colors
            ), 'Number of supplied colors {} disagrees with supplied ncolor: {}'.format(
                len(colors), ncolors)
...
def build_color_mask(
    ...
    color_mask = np.zeros(shape=(nrows, ncols, 4),
...
def pil_grid(images: List[Image.Image], max_horiz: int) -> Image.Image:
    ...
    h_sizes, v_sizes = np.cumsum([0] + h_sizes), np.cumsum(
        [0] + v_sizes)  # type: ignore
