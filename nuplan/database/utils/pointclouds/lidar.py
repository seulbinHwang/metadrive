...
def pcd_to_numpy(pcd_file: str) -> npt.NDArray[np.float32]:
    ...
    assert all(
        f in fields for f in ['x', 'y', 'z']), 'x, y, and z fields are required'
...
class LidarPointCloud:
    ...
    @staticmethod
    def load_pcd_bin(pcd_bin: Union[str, IO[Any], ByteString],
                     pcd_bin_version: int = 1) -> npt.NDArray[np.float32]:
        """
        ...
        if pcd_bin_version == 1:
            ...
            points = np.hstack((points, -1 * np.ones(
    ...
    @staticmethod
    def load_pcd(
            pcd_data: Union[IO[Any], ByteString]) -> npt.NDArray[np.float32]:
        """
        Loads a pcd file.
    ...
    @classmethod
    def from_buffer(cls,
                    pcd_data: Union[IO[Any], ByteString],
                    content_type: str = 'bin') -> LidarPointCloud:
        """
        ...
        if content_type == 'bin':
            ...
            raise NotImplementedError('Not implemented content type: %s' %
                                      content_type)
    ...
    def make_random(cls) -> LidarPointCloud:
        ...
        """
        return LidarPointCloud(points=np.random.normal(
            0, 100, size=(4, 100)))  # type: ignore
    ...
    def range_filter(
        ...
        keep_x = np.logical_and(xrange[0] <= self.points[0], self.points[0]
                                <= xrange[1])
        keep_y = np.logical_and(yrange[0] <= self.points[1], self.points[1]
                                <= yrange[1])
        keep_z = np.logical_and(zrange[0] <= self.points[2], self.points[2]
                                <= zrange[1])
    ...
    def rotate(self, quaternion: Quaternion) -> None:
        ...
        """
        self.points[:3] = np.dot(quaternion.rotation_matrix.astype(np.float32),
                                 self.points[:3])
    ...
    def render_image(
        ...
        view: npt.NDArray[np.float64] = np.array([[10, 0, 0, 500],
                                                  [0, 10, 0, 500],
                                                  [0, 0, 10, 0]]),
        color_dim: int = 2,
    ) -> Image.Image:
        ...
        color_values = 255.0 * (color_values - np.amin(color_values)) / (
            np.amax(color_values) - np.amin(color_values))
        ...
        render = np.tile(
            np.expand_dims(np.zeros(canvas_size, dtype=np.uint8), axis=2),
            [1, 1, 3])  # type: ignore
        color_value_array: npt.NDArray[np.float64] = -1 * np.ones(
            canvas_size, dtype=float)  # type: ignore
    ...
    def render_height(
        ...
        """
        self._render_helper(self.points[2, :], ax, view, x_lim, y_lim,
                            marker_size)
    ...
    def render_intensity(
        ...
        :param marker_size: Marker size.
        """
        self._render_helper(self.points[3, :], ax, view, x_lim, y_lim,
                            marker_size)
    ...
    def render_label(
        ...
        color_list = list(
            map(lambda x: colors.get(x, np.array((1.0, 1.0, 1.0, 0.0))),
                label))  # type: ignore

        self._render_helper(color_list, ax, view, x_lim, y_lim,
                            marker_size)  # type: ignore
