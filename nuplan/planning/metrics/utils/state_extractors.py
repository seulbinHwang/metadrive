...
def calculate_ego_progress_to_goal(ego_states: List[EgoState],
                                   goal: StateSE2) -> Any:
    """
...
def get_ego_distance_to_goal(ego_states: List[EgoState],
                             goal: StateSE2) -> List[np.float64]:
    """
...
def extract_ego_time_point(ego_states: List[EgoState]) -> npt.NDArray[np.int32]:
    ...
    """
    time_point: npt.NDArray[np.int32] = np.array(
        [ego_state.time_point.time_us for ego_state in ego_states])
    return time_point


def extract_ego_x_position(
        history: SimulationHistory) -> npt.NDArray[np.float32]:
    """
    ...
    """
    x: npt.NDArray[np.float32] = np.array(
        [sample.ego_state.rear_axle.x for sample in history.data])
    return x


def extract_ego_y_position(
        history: SimulationHistory) -> npt.NDArray[np.float32]:
    """
    ...
    y: npt.NDArray[np.float32] = np.array(
        [sample.ego_state.rear_axle.y for sample in history.data])
    return y
...
def extract_ego_center(ego_states: List[EgoState]) -> List[Point2D]:
    ...
    """
    xy_poses: List[Point2D] = [
        ego_state.center.point for ego_state in ego_states
    ]
    return xy_poses


def extract_ego_center_with_heading(
        ego_states: List[EgoState]) -> List[StateSE2]:
    """
    Extract xy position of center and heading from a list of ego_states
    :param ego_states: list of ego states
    :return a list of StateSE2.
    """
    xy_poses_and_heading: List[StateSE2] = [
        ego_state.center for ego_state in ego_states
    ]
...
def extract_ego_heading(ego_states: List[EgoState]) -> npt.NDArray[np.float32]:
    ...
    """
    heading: npt.NDArray[np.float32] = np.array(
        [ego_state.rear_axle.heading for ego_state in ego_states])
    return heading
...
def extract_ego_velocity(ego_states: List[EgoState]) -> npt.NDArray[np.float32]:
    ...
    """
    velocity: npt.NDArray[np.float32] = np.array(
        [ego_state.dynamic_car_state.speed for ego_state in ego_states])
    return velocity
...
def extract_ego_jerk(
    ...
    ego_acceleration = extract_ego_acceleration(
        ego_states=ego_states, acceleration_coordinate=acceleration_coordinate)
...
def extract_ego_yaw_rate(
    ...
    ego_yaw_rate = approximate_derivatives(
        phase_unwrap(ego_headings),
        ego_timestamps / 1e6,
        deriv_order=deriv_order,
        poly_order=poly_order)  # convert to seconds
    ego_yaw_rate = np.round(ego_yaw_rate, decimals=decimals)
    return ego_yaw_rate


def extract_ego_tire_steering_angle(
        history: SimulationHistory) -> npt.NDArray[np.float32]:
    """
...
def longitudinal_projection(
        state_vectors: npt.NDArray[np.float32],
    ...
    projection: npt.NDArray[np.float32] = (
        np.cos(headings) * state_vectors[:, 0] +
...
def lateral_projection(
    ...
    """
    projection: npt.NDArray[np.float32] = (
        -np.sin(headings) * state_vectors[:, 0] +
        np.cos(headings) * state_vectors[:, 1])
    return projection


def ego_delta_v_collision(ego_state: EgoState,
    ...
    scene_object_speed = scene_object.velocity.magnitude() if isinstance(
        scene_object, Agent) else 0

    sum_speed_squared = ego_state.dynamic_car_state.speed**2 + scene_object_speed**2
    cos_rule_term = (
        2 * ego_state.dynamic_car_state.speed * scene_object_speed *
...
def extract_tracks_poses(
        history: SimulationHistory) -> List[npt.NDArray[np.float32]]:
    """
    ...
    try:
        for sample in history.data:
            poses: List[npt.NDArray[np.float32]] = [
                np.array([*tracked_object.center])
...
def extract_tracks_speed(
    ...
    for sample in history.data:
        speeds = [
            np.array(tracked_object.velocity.magnitude()) if isinstance(
                tracked_object, Agent) else 0
            for tracked_object in sample.observation.tracked_objects
        ]
