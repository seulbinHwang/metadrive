...
class LQRTracker(AbstractTracker):
    ...
    def __init__(
        ...
        """
        q_longitudinal = np.array([10.], dtype=np.float64)
        ...
        assert len(q_longitudinal
                  ) == 1, "q_longitudinal should have 1 element (velocity)."
        assert len(r_longitudinal
                  ) == 1, "r_longitudinal should have 1 element (acceleration)."
        ...
        assert len(
            q_lateral
        ) == 3, "q_lateral should have 3 elements (lateral_error, heading_error, steering_angle)."
        assert len(
            r_lateral) == 1, "r_lateral should have 1 element (steering_rate)."
        ...
        for attr in ["_q_lateral", "_q_longitudinal"]:
            assert np.all(np.diag(getattr(self, attr)) >=
                          0.0), f"self.{attr} must be positive semidefinite."

        for attr in ["_r_lateral", "_r_longitudinal"]:
            assert np.all(np.diag(getattr(self, attr)) >
                          0.0), f"self.{attr} must be positive definite."
    ...
    def track_trajectory(
        ...
        should_stop = (reference_velocity <= self._stopping_velocity
                      ) and initial_velocity <= self._stopping_velocity

        if should_stop:
            accel_cmd, steering_rate_cmd = self._stopping_controller(
                initial_velocity, reference_velocity)
        else:
            accel_cmd = self._longitudinal_lqr_controller(
                initial_velocity, reference_velocity)
            velocity_profile = _generate_profile_from_initial_condition_and_derivatives(
                initial_condition=initial_velocity,
                derivatives=np.ones(self._tracking_horizon, dtype=np.float64) *
                accel_cmd,
    ...
    def _compute_initial_velocity_and_lateral_state(
        ...
        """
        # sampled_trajectory_first: nuplan.common.actor_state.ego_state.EgoState
        sampled_trajectory_first = trajectory.get_sampled_trajectory()[0]
        # Get initial trajectory state.
        initial_trajectory_state = trajectory.get_state_at_time(
            current_iteration.time_point)
        ...
        lateral_error = -x_error * np.sin(heading_reference) + y_error * np.cos(
            heading_reference)
        heading_error = angle_diff(initial_state.rear_axle.heading,
                                   heading_reference, 2 * np.pi)
    ...
    def _compute_reference_velocity_and_curvature_profile(
        ...
        times_s, poses = get_interpolated_reference_trajectory_poses(
            trajectory, self._discretization_time)
        ...
        profile_times = [
            current_iteration.time_point.time_s + x * self._discretization_time
            for x in range(self._tracking_horizon)
        ]
        reference_curvature_profile = np.interp(profile_times, times_s[:-1],
                                                curvature_profile)
    ...
    def _stopping_controller(
        ...
        """
        accel = -self._stopping_proportional_gain * (initial_velocity -
                                                     reference_velocity)
        return accel, 0.0
    ...
    def _longitudinal_lqr_controller(
        ...
        B: npt.NDArray[np.float64] = np.array(
            [self._tracking_horizon * self._discretization_time],
            dtype=np.float64)
    ...
    def _lateral_lqr_controller(
        ...
        B: npt.NDArray[np.float64] = np.zeros((n_lateral_states, 1),
                                              dtype=np.float64)
        g: npt.NDArray[np.float64] = np.zeros(n_lateral_states,
                                              dtype=np.float64)
        ...
        input_matrix: npt.NDArray[np.float64] = np.zeros((n_lateral_states, 1),
                                                         np.float64)
        input_matrix[idx_steering_angle] = self._discretization_time

        for index_step, (velocity, curvature) in enumerate(
                zip(velocity_profile, curvature_profile)):
            state_matrix_at_step: npt.NDArray[np.float64] = np.eye(
                n_lateral_states, dtype=np.float64)
            state_matrix_at_step[
                idx_lateral_error,
                idx_heading_error] = velocity * self._discretization_time
            state_matrix_at_step[idx_heading_error, idx_steering_angle] = (
                velocity * self._discretization_time / self._wheel_base)

            affine_term: npt.NDArray[np.float64] = np.zeros(n_lateral_states,
                                                            dtype=np.float64)
            affine_term[
                idx_heading_error] = -velocity * curvature * self._discretization_time
    ...
    def _solve_one_step_lqr(
        ...
        lqr_input = -np.linalg.inv(B.T @ Q @ B +
                                   R) @ B.T @ Q @ state_error_zero_input
        return lqr_input
