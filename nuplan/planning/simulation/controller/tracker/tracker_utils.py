...
def _generate_profile_from_initial_condition_and_derivatives(
        initial_condition: float, derivatives: DoubleMatrix,
        discretization_time: float) -> DoubleMatrix:
    """
    ...
    profile = initial_condition + np.insert(
        np.cumsum(derivatives * discretization_time), 0, 0.0)

    return profile  # type: ignore


def _get_xy_heading_displacements_from_poses(
        poses: DoubleMatrix) -> Tuple[DoubleMatrix, DoubleMatrix]:
    """
    ...
    """
    assert len(poses.shape
              ) == 2, "Expect a 2D matrix representing a trajectory of poses."
    assert poses.shape[
        0] > 1, "Cannot get displacements given an empty or single element pose trajectory."
    assert poses.shape[
        1] == 3, "Expect pose to have three elements (x, y, heading)."
...
def _make_banded_difference_matrix(number_rows: int) -> DoubleMatrix:
    ...
    """
    banded_matrix: DoubleMatrix = -1.0 * np.eye(number_rows + 1,
                                                dtype=np.float64)[:-1, :]
...
def _convert_curvature_profile_to_steering_profile(
    ...
    steering_rate_profile = np.diff(
        steering_angle_profile) / discretization_time

    return steering_angle_profile, steering_rate_profile


def _fit_initial_velocity_and_acceleration_profile(
        xy_displacements: DoubleMatrix, heading_profile: DoubleMatrix,
        discretization_time: float,
        jerk_penalty: float) -> Tuple[float, DoubleMatrix]:
    """
    ...
    assert len(
        xy_displacements.shape) == 2, "Expect xy_displacements to be a matrix."
    assert xy_displacements.shape[
        1] == 2, "Expect xy_displacements to have 2 columns."
    ...
    A: DoubleMatrix = np.zeros((2 * num_displacements, num_displacements),
                               dtype=np.float64)
...
def _fit_initial_curvature_and_curvature_rate_profile(
    ...
    A: DoubleMatrix = np.tri(len(y),
                             dtype=np.float64)  # lower triangular matrix
...
def compute_steering_angle_feedback(pose_reference: DoubleMatrix,
                                    pose_current: DoubleMatrix,
                                    lookahead_distance: float,
                                    k_lateral_error: float) -> float:
    """
    ...
    return float(-k_lateral_error *
                 (lateral_error + lookahead_distance * heading_error))
...
def get_velocity_curvature_profiles_with_derivatives_from_poses(
    ...
    xy_displacements, heading_displacements = _get_xy_heading_displacements_from_poses(
        poses
    )  # poses: (N, 3) -> xy_displacements: (N-1, 2), heading_displacements: (N-1,)
    ...
    initial_velocity, acceleration_profile = _fit_initial_velocity_and_acceleration_profile(
        xy_displacements=xy_displacements,  # (N-1, 2)
        heading_profile=poses[:-1, 2],  # (N-1, 3) -> (N-1,)
        discretization_time=discretization_time,  # s
...
def complete_kinematic_state_and_inputs_from_poses(
    ...
    acceleration_profile = np.append(acceleration_profile,
                                     acceleration_profile[-1])
    steering_rate_profile = np.append(steering_rate_profile,
                                      steering_rate_profile[-1])

    velocity_profile = np.append(
        velocity_profile,
        velocity_profile[-1] + acceleration_profile[-1] * discretization_time)
    steering_angle_profile = np.append(
        steering_angle_profile, steering_angle_profile[-1] +
        steering_rate_profile[-1] * discretization_time)

    # Collect completed state and input in matrices.
    kinematic_states: DoubleMatrix = np.column_stack(
        (poses, velocity_profile, steering_angle_profile))
    kinematic_inputs: DoubleMatrix = np.column_stack(
        (acceleration_profile, steering_rate_profile))
...
def get_interpolated_reference_trajectory_poses(
    ...
    interpolation_times_us = np.arange(start_time_point.time_us,
                                       end_time_point.time_us,
                                       delta_time_point.time_us)

    # Adds extra state if it aligns with discretization time
    if interpolation_times_us[
            -1] + delta_time_point.time_us <= end_time_point.time_us:
        interpolation_times_us = np.append(
            interpolation_times_us,
            interpolation_times_us[-1] + delta_time_point.time_us)

    interpolation_time_points = [
        TimePoint(t_us) for t_us in interpolation_times_us
    ]
