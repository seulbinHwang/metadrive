...
class SimulationHistory:
    ...
    def __init__(self,
                 ...
                 data: Optional[List[SimulationHistorySample]] = None) -> None:
        """
        ...
        self.data: List[
            SimulationHistorySample] = data if data is not None else list()
    ...
    def interval_seconds(self) -> float:
        ...
        if not self.data or len(self.data) < 1:
            ...
            raise ValueError(
                "Can't calculate the interval of a single-iteration simulation."
            )

        return float(self.data[1].iteration.time_s -
                     self.data[0].iteration.time_s)  # float cast is for mypy
