...
def _get_fixed_timesteps(state: EgoState, future_horizon: float,
...
def _project_from_global_to_ego_centric_ds(
        ego_poses: npt.NDArray[np.float32],
        values: npt.NDArray[np.float32]) -> npt.NDArray[np.float32]:
    """
    ...
    values_lon = values[:, :1] * np.cos(headings) + values[:, 1:2] * np.sin(
        headings)
    values_lat = values[:, :1] * np.sin(headings) - values[:, 1:2] * np.cos(
        headings)
...
def _get_velocity_and_acceleration(
    ego_poses: List[StateSE2],  # 길이 T_future
    ...
    ego_poses_current_planned: npt.NDArray[
        np.float32] = np.stack([current_ego_state.rear_axle.serialize()] +
                               [pose.serialize() for pose in ego_poses])

    # Interpolation to have equal space for derivatives
    ego_poses_interpolate = interp1d(timesteps_current_planned,
                                     ego_poses_current_planned,
                                     axis=0,
                                     fill_value='extrapolate')
    timesteps_current_planned_interp = np.arange(
        start=current_ego_state.time_point.time_s,
    ...
    timesteps_past_current_planned = [
        *timesteps_past_current, *timesteps_current_planned_interp[1:]
    ]
    ...
    ego_velocity_planned_xy = ego_velocity_past_current_planned[
        ego_history_len:]
...
def _get_absolute_agent_states_from_numpy_poses(
        poses: npt.NDArray[np.float32], ego_history: Deque[EgoState],
        timesteps: List[float]) -> List[EgoState]:
    """
    Converts an array of relative numpy poses to a list of absolute EgoState objects.

    :param poses: input relative poses # (T, 3)
    ...
    relative_states: List[StateSE2] = [
        StateSE2.deserialize(pose) for pose in poses
    ]
    absolute_states: List[StateSE2] = relative_to_absolute_poses(
        ego_state.rear_axle, relative_states)
    """
    여기서 “과거+현재+미래 포즈 시계열”을 
    일정한 시간 간격으로 보간(interpolation) & 미분 → 대략적인 속도·가속도를 추정.
    Ego 좌표계 기준
        velocities: shape (planned steps, 2)
        accelerations: shape (planned steps, 2)
    """
    velocities, accelerations = _get_velocity_and_acceleration(
        ego_poses=absolute_states, ego_history=ego_history, timesteps=timesteps)
    agent_states = [
